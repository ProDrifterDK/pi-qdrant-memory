...
function defaultConfig(host: HostId): DefaultConfig {
  return {
    enabled: true,
    autoRecall: true,
    qdrant: {
...
function assertKeys(value: unknown, allowed: ReadonlySet<string>, path: string): asserts value is JsonRecord {
  if (!isRecord(value)) throw new Error(`${path} must be an object`);
  for (const key of Object.keys(value)) {
    if (DANGEROUS_KEYS.has(key)) throw new Error(`Unsafe configuration field ${path}.${key}`);
    if (!allowed.has(key)) throw new Error(`Unknown configuration field ${path}.${key}`);
  }
}

function validateDestinationEntries(value: unknown, path: string): void {
  if (value === undefined) return;
  if (!Array.isArray(value)) throw new Error(`${path} must be an array`);
  value.forEach((entry, index) => assertKeys(entry, new Set(["id", "residency", "dataUse"]), `${path}[${index}]`));
}

function validateFileShape(root: JsonRecord): void {
  assertKeys(root, ROOT_KEYS, "configuration");
  const validateHost = (value: unknown, path: string): void => {
    if (value === undefined) return;
    ...
    for (const sectionName of Object.keys(SECTION_KEYS)) {
      ...
      if (sectionName === "projects" && isRecord(sectionValue.registrations)) {
        for (const [key, registration] of Object.entries(sectionValue.registrations)) {
          if (DANGEROUS_KEYS.has(key)) throw new Error(`Unsafe configuration field ${path}.${sectionName}.registrations.${key}`);
  ...
  for (const sectionName of Object.keys(SECTION_KEYS)) {
    ...
    if (sectionName === "projects" && isRecord(sectionValue.registrations)) {
      for (const [key, registration] of Object.entries(sectionValue.registrations)) {
        if (DANGEROUS_KEYS.has(key)) throw new Error(`Unsafe configuration field configuration.projects.registrations.${key}`);
...
function inspectFile(value: unknown, path = ""): void {
  ...
  for (const [key, child] of Object.entries(value)) {
    ...
    if (DANGEROUS_KEYS.has(key)) throw new Error("Unsafe configuration field");
    const normalized = key.toLowerCase().replace(/[^a-z0-9]/g, "");
    if (isCredentialKey(normalized)) throw new Error("Credentials are allowed only through environment variables");
...
function environmentRaw(env: Record<string, string | undefined>, name: string, fallback: unknown): unknown {
  return env[`${PREFIX}${name}`] ?? fallback;
}

function environmentSecret(env: Record<string, string | undefined>, name: string): string | undefined {
...
function checkEnvironmentNames(env: Record<string, string | undefined>): void {
  for (const name of Object.keys(env)) {
    if (!name.startsWith(PREFIX)) continue;
    const suffix = name.slice(PREFIX.length);
    if (suffix === "HOST") continue;
    if (suffix.startsWith(RETIRED_ENV_PREFIX) || OPERATIONAL_SUFFIXES.has(suffix) || SECRET_SUFFIXES.has(suffix)) {
      if (OPERATIONAL_SUFFIXES.has(suffix) || SECRET_SUFFIXES.has(suffix)) continue;
...
function retention(name: string, raw: unknown, fallback: RuntimeConfig["capture"]["episodeRetentionDays"]): RuntimeConfig["capture"]["episodeRetentionDays"] {
  if (raw === undefined) return fallback;
  if (raw === "indefinite") return raw;
  return boundedInteger(name, raw, 1, 3650);
}

function consistency(name: string, raw: unknown, fallback: RuntimeConfig["coordination"]["readConsistency"]): RuntimeConfig["coordination"]["readConsistency"] {
  if (raw === undefined) return fallback;
...
export async function loadConfig(host: HostId, deps: ConfigLoadDependencies): Promise<RuntimeConfig> {
  ...
  const retrieval = mergeRecords(defaults.retrieval as unknown as JsonRecord, section(file, "retrieval", "retrieval configuration"), section(hostOverrides, "retrieval", `${host}.retrieval configuration`));
