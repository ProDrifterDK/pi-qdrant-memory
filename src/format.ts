...
function provenance(hit: MemoryCandidate): string {
  ...
  const labels: string[] = [`project=${project}`, `type=${sourceType}`, `system=${sourceSystem}`];
  if (hit.scope !== undefined) labels.push(`scope=${formatMemoryProvenance(hit.scope)}`);
  if (hit.createdAt !== undefined) labels.push(`date=${formatMemoryProvenance(hit.createdAt)}`);
  if (hit.validFrom !== undefined) labels.push(`valid_from=${formatMemoryProvenance(hit.validFrom)}`);
  if (hit.validTo !== undefined) labels.push(`valid_to=${formatMemoryProvenance(hit.validTo)}`);
  if (hit.policyEpoch !== undefined && Number.isSafeInteger(hit.policyEpoch)) labels.push(`policy_epoch=${hit.policyEpoch}`);
