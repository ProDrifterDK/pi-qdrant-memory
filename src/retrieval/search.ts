...
function vector(value: unknown): number[] | undefined {
  ...
  if (!isRecord(value) || Object.keys(value).length !== 1 || !Array.isArray(value.semantic) || value.semantic.length !== 1024 || !value.semantic.every((part) => typeof part === "number" && Number.isFinite(part))) invalid("Qdrant semantic vector is invalid");
  return [...value.semantic] as number[];
}
...
export class GuardedMemoryReadStore implements MemoryReadStore {
  ...
  async #retrieve(refs: readonly MemoryRecordRef[], signal?: AbortSignal): Promise<MemoryRecord[]> {
    ...
    const raw = await fetchJson<unknown>(url(this.#options, "/points/retrieve"), { method: "POST", headers: headers(this.#options), body: JSON.stringify({ ids, with_payload: true, with_vector: true }) }, requestOptions(this.#options, signal));
  ...
  async retrieveEvidence(ids: readonly string[]): Promise<MemoryRecord[]> {
    ...
    const raw = await fetchJson<unknown>(url(this.#options, "/points/retrieve"), { method: "POST", headers: headers(this.#options), body: JSON.stringify({ ids: physicalIds, with_payload: true, with_vector: true }) }, requestOptions(this.#options));
