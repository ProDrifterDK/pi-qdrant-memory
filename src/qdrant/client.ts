...
export interface QdrantReadPolicy { ownerHost: HostId; purpose: ReadPurpose; recordTypes: readonly PointRecordType[]; now: number; maxClockSkewMs: number; requireStatus: "active"; requireSecretScan: "passed"; projectId?: string; processingPolicyId?: string; privacyEpoch?: number; }
...
export interface QdrantCollectionInfo { status?: string; dimension: 1024; distance: "Dot"; vectors: { semantic: { size: 1024; distance: "Dot" } }; pointsCount: number | null; payloadSchema?: JsonRecord; raw: unknown; }
...
export interface PreparedPoint { id: PointId; payload: JsonRecord; vector?: { semantic: readonly number[] } | Record<string, never>; }
...
export function readPolicy(input: { ownerHost: HostId; purpose: ReadPurpose; recordTypes: readonly PointRecordType[]; now?: number; maxClockSkewMs?: number; projectId?: string; processingPolicyId?: string; privacyEpoch?: number }): QdrantReadPolicy {
  ...
  if (projectId !== undefined && (typeof projectId !== "string" || projectId.length === 0) || processingPolicyId !== undefined && (typeof processingPolicyId !== "string" || processingPolicyId.length === 0) || privacyEpoch !== undefined && (!Number.isSafeInteger(privacyEpoch) || privacyEpoch < 0)) throw new TypeError("Read scope policy is invalid");
  ...
  if (privacyEpoch !== undefined) policy.privacyEpoch = privacyEpoch;
  return policy;
}
