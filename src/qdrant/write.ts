...
async function completeJobOnProtocol(protocol: CoordinationStore, target: ProductionCoordinationStore, scope: object, authority: LeaseAuthority): Promise<boolean> {
  ...
  for (let projectionIndex = 0; projectionIndex < projections.length; projectionIndex += 1) {
    ...
    if (current === null || current.recordType !== "curated_current" || current.id !== projection.currentId || current.ownerHost !== target.ownerHost || current.coordinationPolicyEpoch !== job.coordinationPolicyEpoch || current.coordinationPolicyHash !== job.coordinationPolicyHash || current.privacyEpoch !== job.privacyEpoch || current.stateKey !== projection.stateKey || (current.sourceEpisodeIds ?? []).length === 0 || current.contentHash !== canonicalRecordHash(current)) return no("current-envelope");
