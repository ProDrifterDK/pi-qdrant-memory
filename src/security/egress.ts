...
import { redactAndScan, type SecretScanner } from "./redaction.js";
...
export interface CuratedEgressOptions { maxChars?: number; homeDir?: string; scan?: SecretScanner; }
/**
 ...
 */
export function gateCuratedEgressText(text: string, options: CuratedEgressOptions = {}): RedactedEgressMaterial {
  if (typeof text !== "string" || text.length === 0) throw new TypeError("Curated egress text must be a bounded non-empty string");
  const maxChars = options.maxChars ?? 4096;
  const homeDir = options.homeDir ?? "/";
  if (!Number.isSafeInteger(maxChars) || maxChars < 0 || maxChars > 16_000 || typeof homeDir !== "string" || homeDir.length === 0 || homeDir.length > 4096) throw new TypeError("Curated egress options are invalid");
  const checked = redactAndScan({ text, maxChars, homeDir, ...(options.scan === undefined ? {} : { scan: options.scan }) });
  if (checked.dropped || checked.secretScan !== "passed" || checked.text.length === 0) throw new TypeError("Curated egress text failed structural redaction or the final secret scan");
  if (checked.redactionStatus !== "unchanged" && checked.redactionStatus !== "redacted") throw new TypeError("Curated egress text redaction status is invalid");
  const material: RedactedEgressMaterial = { text: checked.text, contentHash: checked.contentHash, redactionStatus: checked.redactionStatus, secretScan: checked.secretScan, dropped: false };
...
export function bindConfiguredDestination(input: {
  ...
  const valid = (destination: AuthorizedDestination): void => {
    ...
    if (!derivedLocal) { const checked = redactAndScan({ text: destination.id, maxChars: 256, homeDir: "/" }); if (checked.dropped || checked.redactionStatus !== "unchanged" || checked.text !== destination.id) throw new TypeError("Configured destination identity is unsafe"); }
