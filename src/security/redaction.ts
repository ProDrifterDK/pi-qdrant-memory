...
const KNOWN_TOKEN_PATTERNS: readonly [RegExp, string][] = [
...
function marker(kind: string): string { return `[${kind} redacted]`; }
function kindFor(name: string): string {
  const lower = name.toLowerCase();
  if (lower.includes("cookie")) return "cookie";
  if (lower.includes("authoriz") || lower === "bearer") return "authorization";
  if (lower.includes("api") || lower.includes("key")) return "api_key";
  if (lower.includes("password") || lower.includes("passwd")) return "password";
  if (lower.includes("secret") || lower.includes("credential") || lower.includes("private")) return "secret";
  return "token";
}
function isPlaceholder(value: string): boolean { return PLACEHOLDER.test(value.trim()); }
...
function canonicalizeHome(value: string, homeDir: string): string {
  if (!isAbsolute(homeDir) || homeDir === "/") return value;
  ...
  while (cursor < value.length) {
    ...
    if (beforeOk && afterOk) { output += value.slice(cursor, found) + "$HOME"; cursor = found + home.length; }
    else { output += value.slice(cursor, found + home.length); cursor = found + home.length; }
  }
  return output;
}
function redactUrls(value: string): { value: string; changed: boolean } {
  let changed = false;
  const result = value.replace(URL_PATTERN, (raw) => {
    ...
    if (parsed.username !== "" || parsed.password !== "") changed = true;
    parsed.username = "";
    parsed.password = "";
    const pairs: string[] = [];
    ...
    for (const [key, entry] of parsed.searchParams.entries()) {
      if (sensitiveName(decodeURIComponent(key.replace(/\+/gu, " ")))) {
        ...
        queryChanged = true;
      } else pairs.push(`${encodeURIComponent(key)}=${encodeURIComponent(entry)}`);
    }
    if (queryChanged) changed = true;
    const hasCredentials = parsed.username !== "" || parsed.password !== "" || candidate.includes("@");
    if (!hasCredentials && !queryChanged) return raw;
...
function redactJsonValue(value: unknown): { changed: boolean; failed: boolean } {
  const stack: Array<{ value: object; depth: number }> = [{ value: value as object, depth: 0 }];
  let changed = false; let nodes = 0;
  while (stack.length > 0) {
    const current = stack.pop()!;
    nodes += 1;
    if (nodes > MAX_JSON_NODES || current.depth > MAX_JSON_DEPTH) return { changed: false, failed: true };
    for (const [name, child] of Object.entries(current.value as Record<string, unknown>)) {
      if (sensitiveName(name) && !(typeof child === "string" && isPlaceholder(child))) {
        (current.value as Record<string, unknown>)[name] = marker(kindFor(name)); changed = true; continue;
      }
...
function redactJsonText(value: string): { value: string; changed: boolean; failed: boolean } {
  const trimmed = value.trim();
  if (!trimmed.startsWith("{") && !trimmed.startsWith("[")) return { value, changed: false, failed: false };
  let parsed: unknown;
  try { parsed = JSON.parse(trimmed); }
  catch (error) { return { value, changed: false, failed: error instanceof RangeError }; }
  if (parsed === null || typeof parsed !== "object") return { value, changed: false, failed: false };
  const result = redactJsonValue(parsed);
  if (result.failed) return { value: "", changed: false, failed: true };
  if (!result.changed) return { value, changed: false, failed: false };
  try {
  ...
  } catch { return { value: "", changed: false, failed: true }; }
}

function redactStructuredFields(value: string): { value: string; changed: boolean; failed: boolean } {
  let changed = false;
  let result = value;
  const json = redactJsonText(result);
  if (json.failed) return { value: "", changed: false, failed: true };
  changed ||= json.changed; result = json.value;
  // Quoted JSON/YAML-like fields. The value is intentionally field-local and bounded.
  result = result.replace(/(["'])([^"']+)\1\s*:\s*(["'])([^"']*)\3/gu, (whole, quote: string, name: string, valueQuote: string, fieldValue: string) => {
    if (!sensitiveName(name) || isPlaceholder(fieldValue)) return whole;
    changed = true;
  ...
  result = result.replace(/\b([A-Za-z][A-Za-z0-9_.-]{1,80})\s*(?::|=)\s*(?:Bearer\s+)?(?:"([^"]*)"|'([^']*)'|(\[[^\]\n]{1,200}\]|<[^>\n]{1,200}>|[^\s,;}\]\n]+))/giu, (whole, name: string, doubleValue?: string, singleValue?: string, bareValue?: string) => {
    if (!sensitiveName(name)) return whole;
    const fieldValue = doubleValue ?? singleValue ?? bareValue ?? "";
    if (isPlaceholder(fieldValue)) return whole;
    changed = true;
...
function redactKnownTokens(value: string): { value: string; changed: boolean } {
  let changed = false;
  let result = value;
 ...
 */
function structuralRedact(input: { text: string; maxChars: number; homeDir: string }): StructuralRedactionResult {
  let text: string;
  ...
  try {
    if (typeof input.text !== "string" || !Number.isSafeInteger(input.maxChars) || input.maxChars < 0 || input.maxChars > MAX_HARD_CHARS || typeof input.homeDir !== "string") throw new TypeError("invalid redaction input");
    text = input.text.normalize("NFC");
    if (text !== input.text) changed = true;
    const normalized = normalizeControls(text); changed ||= normalized !== text; text = normalized;
    const withHome = canonicalizeHome(text, input.homeDir); changed ||= withHome !== text; text = withHome;
    const fields = redactStructuredFields(text); if (fields.failed) return { text: "", redactionStatus: "dropped", contentHash: emptyHash() }; changed ||= fields.changed; text = fields.value;
    const urls = redactUrls(text); changed ||= urls.changed; text = urls.value;
    const known = redactKnownTokens(text); changed ||= known.changed; text = known.value;
    const bounded = boundedText(text, Math.min(input.maxChars, MAX_HARD_CHARS)); changed ||= bounded.truncated; text = bounded.value;
    if (text.length === 0) return { text: "", redactionStatus: "dropped", contentHash: emptyHash() };
...
export function redactStructure(input: { text: string; maxChars: number; homeDir: string }): StructuralRedactionResult {
  return structuralRedact(input);
}
export function redactAndScan(input: RedactAndScanInput): RedactionResult {
  const structural = structuralRedact(input);
  const emptyHash = createHash("sha256").update("", "utf8").digest("hex");
  if (structural.redactionStatus === "dropped") return { ...structural, text: "", secretScan: "error", dropped: true, contentHash: emptyHash };
  let verdict: SecretScanStatus;
  try {
    ...
    if (verdict === "passed" && input.scan !== undefined) verdict = input.scan(structural.text);
    if (verdict !== "passed" && verdict !== "rejected" && verdict !== "error") verdict = "error";
  } catch { verdict = "error"; }
  if (verdict !== "passed") return { ...structural, text: "", secretScan: verdict, dropped: true, contentHash: emptyHash };
  return { ...structural, secretScan: "passed", dropped: false, contentHash: createHash("sha256").update(structural.text, "utf8").digest("hex") };
}

export function redactField(input: { name: string; value: string; maxChars: number; homeDir: string }): RedactionResult {
