...
function ids(name: string, value: unknown, max = MAX_ARRAY): asserts value is string[] { if (!Array.isArray(value) || value.length === 0 || value.length > max) fail(`${name} must be bounded IDs`); value.forEach((item) => text(name, item, MAX_ID_CHARS, true)); }
...
function validate(value: PlainRecord, context: RecordValidationContext): MemoryRecord {
  ...
  switch (recordType) {
    ...
    case "job": {
      text("policyId", value.policyId); if (value.policyId !== value.processingPolicyId) fail("job policy ID mismatch"); text("policyHash", value.policyHash, MAX_ID_CHARS, false); if (value.policyHash !== value.coordinationPolicyHash) fail("job policy hash mismatch"); integer("policyEpoch", value.policyEpoch); if (value.policyEpoch !== value.coordinationPolicyEpoch) fail("job policy epoch mismatch"); ids("membership", value.membership, 65_536); strictlySorted("job membership", value.membership); text("extractorRevision", value.extractorRevision, MAX_ID_CHARS, true); const jobOwner = value.ownerHost; host("job.ownerHost", jobOwner); integer("job.privacyEpoch", value.privacyEpoch); try { if (value.id !== jobId(jobOwner, value.membership, value.policyHash, value.extractorRevision, value.policyEpoch, value.policyId, value.privacyEpoch)) fail("job ID formula mismatch"); } catch { fail("job ID formula mismatch"); } if (context.policyEpoch !== undefined && value.policyEpoch !== context.policyEpoch) fail("policy epoch mismatch"); return value as unknown as JobRecord;
    }
    case "lease":
      text("jobId", value.jobId); text("ownerId", value.ownerId); integer("version", value.version, 1); integer("fencingToken", value.fencingToken); if (value.expiresAt === null) fail("lease requires an expiry"); isoDate("expiresAt", value.expiresAt); if (value.state !== "leased" && value.state !== "accepted" && value.state !== "released" && value.state !== "completed") fail("lease state invalid"); if (value.acceptedProposalId !== null) text("acceptedProposalId", value.acceptedProposalId); if (value.acceptedManifestHash !== null) text("acceptedManifestHash", value.acceptedManifestHash, MAX_ID_CHARS, false); if ((value.acceptedProposalId === null) !== (value.acceptedManifestHash === null)) fail("lease acceptance fields must move together"); if (value.terminalOperation !== undefined && value.terminalOperation !== "raptor") fail("lease terminal operation is invalid"); if (value.terminalOperation !== undefined && (value.state !== "completed" || value.acceptedProposalId !== null)) fail("lease terminal operation is not bound to a terminal unaccepted claim"); if (value.state === "leased" && value.acceptedProposalId !== null) fail("leased claim cannot carry acceptance"); if ((value.state === "accepted" || value.state === "completed") && value.acceptedProposalId === null && value.terminalOperation !== "raptor") fail("accepted/completed claim requires proposal and manifest"); try { if (value.id !== leasePointId(value.jobId)) fail("lease ID formula mismatch"); } catch { fail("lease ID formula mismatch"); } return value as unknown as LeaseRecord;
    case "proposal": {
