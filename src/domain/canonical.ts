...
/** Values accepted by the canonical JSON encoder. */
...
function normalizeArray(value: unknown[], ancestors: Set<object>): CanonicalValue[] {
  ...
  if (names.length !== value.length + 1 || !names.includes("length")) invalid("a sparse array or array with extra properties");
  const output: CanonicalValue[] = [];
  Object.defineProperty(output, "toJSON", { value: undefined, enumerable: false });
  for (let index = 0; index < value.length; index += 1) {
    const descriptor = Object.getOwnPropertyDescriptor(value, String(index));
    if (descriptor === undefined || !("value" in descriptor) || !descriptor.enumerable) invalid("a sparse array or array accessor");
...
function normalizeObject(value: object, ancestors: Set<object>): { [key: string]: CanonicalValue } {
  ...
  for (const name of Object.getOwnPropertyNames(value).sort()) {
    ...
    if (descriptor === undefined || !("value" in descriptor) || !descriptor.enumerable) invalid("a non-enumerable property or accessor");
 ...
 */
export function deterministicUuid(namespace: string, ...inputs: readonly unknown[]): string {
  if (namespace.length === 0) throw new TypeError("UUID namespace must not be empty");
...
export function deterministicPointId(targetHost: "pi" | "prime", sourceCollection: string, sourceId: string | number): string {
