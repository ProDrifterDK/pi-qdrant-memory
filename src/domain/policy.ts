...
export interface ProcessingPolicy {
  id: string;
  ownerHost: HostId;
  destinationIds: { qdrant: string; embedding: string; llm?: string };
...
function destination(name: string, value: unknown): asserts value is string { bounded(name, value, MAX_ID, true); if (!/^[A-Za-z0-9._:/-]+$/u.test(value)) throw new TypeError(`${name} must be an exact destination ID`); }
function label(name: string, value: unknown): asserts value is string { bounded(name, value, MAX_LABEL, true); if (!/^[A-Za-z0-9._:/ -]+$/u.test(value)) throw new TypeError(`${name} must be a redacted label`); }
function date(name: string, value: unknown): void { bounded(name, value, 24); const parsed = Date.parse(value); if (!DATE_RE.test(value) || !Number.isFinite(parsed) || new Date(parsed).toISOString() !== value) throw new TypeError(`${name} must be a bounded ISO timestamp`); }
function validPolicyShape(policy: ProcessingPolicy): void {
  ...
  for (const key of Object.keys(policy as unknown as Record<string, unknown>)) if (!allowed.has(key)) throw new TypeError("unknown processing policy field");
  bounded("policy.id", policy.id, MAX_ID, true);
  if (policy.ownerHost !== "pi" && policy.ownerHost !== "prime") throw new TypeError("policy ownerHost is invalid");
  if (typeof policy.destinationIds !== "object" || policy.destinationIds === null || Array.isArray(policy.destinationIds)) throw new TypeError("policy destinationIds is invalid");
  ...
  if (policy.destinationIds.llm !== undefined) destination("destinationIds.llm", policy.destinationIds.llm);
  bounded("policy.originProvider", policy.originProvider, MAX_ID, true);
  if (typeof policy.allowCrossProviderReplay !== "boolean") throw new TypeError("policy replay flag is invalid");
  if (policy.expiresAt !== null) date("policy.expiresAt", policy.expiresAt);
  label("policy.residency", policy.residency); label("policy.dataUse", policy.dataUse);
  bounded("policy.policyRevision", policy.policyRevision, MAX_ID, true);
  for (const key of Object.keys(policy.destinationIds)) if (!["qdrant", "embedding", "llm"].includes(key)) throw new TypeError("unknown policy destination capability");
}

export function processingPolicyHash(policy: ProcessingPolicy): string {
  validPolicyShape(policy);
  const { id: _computedId, ...withoutId } = policy;
  return sha256Hex(canonicalStringify(withoutId));
}
function validPolicy(policy: ProcessingPolicy): void {
  validPolicyShape(policy);
  if (policy.id !== processingPolicyHash(policy)) throw new TypeError("processing policy ID is not content addressed");
}

function earliestExpiry(policies: readonly ProcessingPolicy[]): string | null {
...
export function isPolicyExpired(policy: ProcessingPolicy, now = Date.now(), skewMs = 0): boolean {
  validPolicy(policy);
