...
import { contentId, curatedCurrentId, evidenceLinkId, observationId, stateKey, validateEffectiveOrder, MAX_SESSION_SEQUENCE, type EffectiveOrder, type SessionEffectiveOrder } from "../domain/ids.js";
import { canonicalRecordHash, type ConflictManifestRecord, type CoverageRecord, type CuratedCurrentRecord, type CuratedMemoryRecord, type EpisodeRecord, type EvidenceLinkRecord, type JobRecord, type ProposalRecord, type TombstoneRecord } from "../domain/records.js";
...
import { assertPersistableCurationResult, validateCurationResult, type CurationResult } from "./validate.js";
...
export function derivedCuratedText(item: { category: string; scope: string; subject: string; predicate: string; value?: unknown; text?: string; evidence: readonly string[] }): string {
...
function ownData<T>(value: object, key: string, required = true): T {
  const descriptor = Object.getOwnPropertyDescriptor(value, key);
  if (descriptor === undefined) { if (required) throw new TypeError(`Curation input ${key} is missing`); return undefined as T; }
  if (!("value" in descriptor) || descriptor.enumerable !== true) throw new TypeError(`Curation input ${key} must be an own data field`);
  return descriptor.value as T;
}
function ownedJsonSnapshot<T>(value: unknown): T {
  ...
  try { return JSON.parse(canonicalStringify(clone(value))) as T; }
  catch { throw new TypeError("Curation input is not canonical JSON"); }
}
 ...
 */
async function readTombstonesChunked(store: ProductionCoordinationStore, ids: readonly string[]): Promise<readonly TombstoneRecord[]> {
  const found: TombstoneRecord[] = [];
  for (let index = 0; index < ids.length; index += 1024) found.push(...await store.readTombstones(ids.slice(index, index + 1024)));
  return found;
}

async function assertMaterializationLive(store: ProductionCoordinationStore, authority: LeaseAuthority, embeddingDestination: { id: string }, membership: readonly string[], derivedTargets: readonly string[], expectedJob: JobRecord, expectedProposal: ProposalRecord): Promise<void> {
  const membershipSet = new Set(membership);
  const derived = [...new Set(derivedTargets)].filter((target) => !membershipSet.has(target)).sort();
  if (derived.some((target) => typeof target !== "string" || target.length === 0 || target.length > 512) || derived.length > 4096) throw new TypeError("Derived tombstone target set is unbounded");
  const acceptance = await readActiveAcceptance(store, authority);
  if (acceptance === null || canonicalStringify(acceptance.job) !== canonicalStringify(expectedJob) || canonicalStringify(acceptance.proposal) !== canonicalStringify(expectedProposal)) throw new TypeError("Materialization acceptance is stale after the control/lease/proposal barrier");
  const control = await store.readControl();
  if (control.state !== "active" || control.coordinationPolicyEpoch !== authority.coordinationPolicyEpoch || control.coordinationPolicyHash !== authority.coordinationPolicyHash || control.privacyEpoch !== authority.privacyEpoch) throw new TypeError("Materialization control identity changed");
  if (control.revokedDestinationIds.includes(embeddingDestination.id)) throw new TypeError("Materialization embedding destination is revoked");
  ...
  if (tombstones.length > 0 || derivedTombstones.length > 0) throw new TypeError("Materialization membership is tombstoned");
...
function decideCurrent(current: CuratedCurrentRecord | null, observation: CuratedMemoryRecord, maxClockSkewMs: number): CurrentDecision {
  ...
  if (current.processingPolicyId !== observation.processingPolicyId || current.expiresAt !== observation.expiresAt) {
    ...
    else if (comparison === "after") return "late";
    else throw new TypeError("Cross-policy current transition is not strictly causal");
  }
  if (current.resolution === "resolved") {
    ...
    if (current.contentId === observation.contentId) {
      if (comparison === "before") return "update";
      return "converge";
    }
    if (comparison === "within_skew") return "conflict";
    if (comparison === "equal") return "converge";
    if (comparison === "before") return "update";
    return "late";
  }
  ...
  if (comparison === "within_skew") return "conflict_grow";
  if (comparison === "equal") return "converge";
  if (comparison === "before") return "update";
  return "late";
}
 ...
 */
function strictHistoryOrder(left: CuratedMemoryRecord, right: CuratedMemoryRecord): number {
  ...
  const tuple = (value: EffectiveOrder): readonly [string, string, string] | null => {
 ...
 */
export function foldHistorySegments(observations: readonly CuratedMemoryRecord[], maxClockSkewMs: number): readonly HistorySegment[] {
  if (!Number.isSafeInteger(maxClockSkewMs) || maxClockSkewMs < 0) throw new TypeError("Clock skew must be a non-negative integer");
  if (!Array.isArray(observations)) throw new TypeError("Observations must be a bounded list");
...
export async function materializeCuration(authority: LeaseAuthority, input: MaterializeCurationOptions): Promise<MaterializationOutcome> {
  ...
  if (!LeaseAuthority.isValid(authority)) throw new TypeError("Materialization requires a genuine lease authority");
  if (input === null || typeof input !== "object" || Array.isArray(input) || nodeTypes.isProxy(input) || (Object.getPrototypeOf(input) !== Object.prototype && Object.getPrototypeOf(input) !== null)) throw new TypeError("Materialization input is invalid");
  const store = ownData<ProductionCoordinationStore>(input, "store");
  if (!ProductionCoordinationStore.isValid(store)) throw new TypeError("Materialization requires the branded production store");
  if (!authority.matchesStore(store)) throw new TypeError("Materialization authority does not match the store");
  if (authority.state !== "accepted") throw new TypeError("Materialization requires an ACCEPTED lease authority");
  const embedding = ownData<BoundEmbeddingDestination>(input, "embedding");
  if (!BoundEmbeddingDestination.isValid(embedding)) throw new TypeError("Materialization requires the opaque bound embedding destination");
  ...
  const resultInput = ownedJsonSnapshot<CurationResult>(ownData(input, "result"));
  const policy = ownedJsonSnapshot<ProcessingPolicy>(ownData(input, "policy"));
  const extractorRevision = ownData<string>(input, "extractorRevision");
  const scan = ownData<SecretScanner | undefined>(input, "scan", false);
  ...
  if (policyHash !== policy.id || isPolicyExpired(policy, now, maxClockSkewMs) || policy.ownerHost !== host) throw new TypeError("Active policy intersection is invalid");
  ...
  if (policyEmbedding === undefined || embeddingDestination.id !== policyEmbedding || embeddingDestination.residency !== policy.residency || embeddingDestination.dataUse !== policy.dataUse) throw new TypeError("Embedding destination does not match the policy intersection");
  if (embeddingCoordination.policyHash !== authority.coordinationPolicyHash || embeddingCoordination.policyEpoch !== authority.coordinationPolicyEpoch) throw new TypeError("Embedding coordination identity does not match the authority");
  if (policy.id !== authority.processingPolicyId) throw new TypeError("Policy intersection does not match the authority");
  ...
  const acceptance = await readActiveAcceptance(store, authority);
  if (acceptance === null) throw new TypeError("Active acceptance is no longer valid before materialization");
  ...
  if (canonicalStringify(proposedResult) !== canonicalStringify(result)) throw new TypeError("Curation result does not match the accepted proposal");
  ...
  if (new Set(projections.map((projection) => projection.observationId)).size !== projections.length) throw new TypeError("duplicate-observation-projection");
  ...
  if (controlBefore.state !== "active") throw new TypeError("Control is not active before embedding");
  if (controlBefore.revokedDestinationIds.includes(embeddingDestination.id)) throw new TypeError("Embedding destination is revoked before embedding");
  if (controlBefore.coordinationPolicyEpoch !== authority.coordinationPolicyEpoch || controlBefore.coordinationPolicyHash !== authority.coordinationPolicyHash || controlBefore.privacyEpoch !== authority.privacyEpoch) throw new TypeError("Control policy identity changed before embedding");
  const observations: CuratedMemoryRecord[] = [];
  const evidenceLinks: EvidenceLinkRecord[] = [];
  const currents: CuratedCurrentRecord[] = [];
  const conflicts: ConflictManifestRecord[] = [];
  for (let projectionIndex = 0; projectionIndex < result.items.length; projectionIndex += 1) {
    ...
    let material: ReturnType<typeof gateCuratedEgressText>;
    if (existingObservation !== null) {
      ...
      material = { text: existingObservation.text ?? "", redactionStatus: "unchanged", secretScan: "passed", dropped: false, contentHash: sha256Hex(existingObservation.text ?? "") } as ReturnType<typeof gateCuratedEgressText>;
    } else {
    ...
    if (!Array.isArray(vector) || vector.length !== 1024 || !vector.every((value) => typeof value === "number" && Number.isFinite(value))) throw new TypeError("Embedding must yield exactly 1024 finite components");
    ...
    await assertMaterializationLive(store, authority, embeddingDestination, membership, derivedTargets, job, proposal);
    const observation: CuratedMemoryRecord = {
    ...
    if (alreadyMemberConflict) decision = "converge";
    ...
    if (decision === "insert" || decision === "update" || decision === "conflict" || decision === "conflict_grow") {
      ...
      const nextBase = {
        recordType: "curated_current" as const, id: currentId, ownerHost: host, schemaRevision: 1,
      ...
      if (decision === "conflict" || decision === "conflict_grow") {
        ...
        if (members.length < 2) throw new TypeError("Conflict manifest requires at least two observations");
        ...
        if (members.length > MAX_CONFLICT_MEMBERS) throw new TypeError("Conflict manifest member cap exceeded");
        ...
        if (current?.resolution === "resolved" && current.observationId !== undefined && !aggregateMembers.some((member) => member.id === current.observationId)) {
          const pointed = await store.readObservation(authority, current.observationId);
          if (pointed === null) throw new TypeError("Existing conflict observation is missing");
          aggregateMembers.push(pointed);
        }
        const aggregate = projectConflictAggregate(aggregateMembers);
        if (aggregate.members.length !== members.length) throw new TypeError("Conflict aggregate membership is incomplete");
        const manifest: ConflictManifestRecord = {
        ...
        next = { ...nextBase, createdAt: aggregate.createdAt, sourceEpisodeIds: [...aggregate.sourceEpisodeIds], resolution: "conflict", conflictManifestHash: storedManifest.id, effectiveOrder: aggregate.effectiveOrder } as CuratedCurrentRecord;
      } else {
        next = {
      ...
      if (storedCurrent === null) throw new TypeError("Curated current CAS failed (retryable)");
  ...
  await assertMaterializationLive(store, authority, embeddingDestination, membership, derivedTargets, job, proposal);
  const outcome: MaterializationOutcome = { observations: Object.freeze(observations), evidenceLinks: Object.freeze(evidenceLinks), currents: Object.freeze(currents), conflicts: Object.freeze(conflicts), coverage: Object.freeze(coverage) };
  return Object.freeze(outcome);
}

/** Deterministic content-addressed evidence digest for audit trails. */
export function evidenceDigest(evidence: readonly string[]): string {
  if (!Array.isArray(evidence) || evidence.length === 0 || evidence.length > 1024 || evidence.some((id) => typeof id !== "string")) throw new TypeError("Evidence is invalid");
