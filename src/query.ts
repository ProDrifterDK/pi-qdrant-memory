...
function retrievalInstant(value: string): string {
  const match = STRICT_RFC3339.exec(value);
  if (match === null) throw new TypeError("Invalid retrieval time bound");
  ...
  if (!Number.isFinite(millis)) throw new TypeError("Invalid retrieval time bound");
  return new Date(millis).toISOString();
}

/** Strict, model-safe RFC3339 retrieval window. Offsets are accepted but canonicalized to UTC. */
export function parseRetrievalWindow(after?: string, before?: string): RetrievalWindow {
  const result: RetrievalWindow = {};
  if (after !== undefined) result.after = retrievalInstant(after);
  if (before !== undefined) result.before = retrievalInstant(before);
