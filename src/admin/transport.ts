...
/** @internal */ export async function statusRetrieve(options: QdrantClientOptions, fetchImpl: typeof fetch, ids: readonly string[], policy: QdrantReadPolicy, includeVector = false): Promise<QdrantPoint[]> { return readClientFor(options, fetchImpl).retrieve(ids, policy, { includeVector }); }
/** @internal */ export async function statusScroll(options: QdrantClientOptions, fetchImpl: typeof fetch, policy: QdrantReadPolicy, offset?: string, limit = 64): Promise<QdrantScrollResult> { return readClientFor(options, fetchImpl).scroll({ policy, ...(offset === undefined ? {} : { offset }), limit }); }
/** @internal */ export async function statusCount(options: QdrantClientOptions, fetchImpl: typeof fetch, policy: QdrantReadPolicy): Promise<number> { return readClientFor(options, fetchImpl).count(policy); }
