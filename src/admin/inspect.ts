...
import type { PointRecordType } from "../qdrant/schema.js";
...
function safeId(value: unknown): string {
  if (typeof value !== "string" || value.length === 0 || value.length > MAX_ID || SECRET.test(value) || /[\u0000-\u001f\u007f-\u009f]/u.test(value)) throw new TypeError("Inspect ID is invalid or not redacted");
  return value;
}
function boundedLimit(value: number | undefined): number { const limit = value ?? 64; if (!Number.isSafeInteger(limit) || limit < 1 || limit > MAX_RECORDS) throw new TypeError("Inspect limit is invalid"); return limit; }
function safeScalar(value: unknown): unknown {
  if (value === null || typeof value === "boolean" || typeof value === "number" && Number.isFinite(value)) return value;
  if (typeof value === "string" && value.length <= MAX_ID && !SECRET.test(value) && !/[\u0000-\u001f\u007f-\u009f]/u.test(value)) return value;
  return undefined;
}
...
export function redactInspectRecord(value: unknown): SafeInspectRecord {
  if (typeof value !== "object" || value === null || Array.isArray(value)) throw new TypeError("Inspect record is invalid");
  const source = value as Record<string, unknown>;
  const output: Record<string, unknown> = {};
  for (const key of Object.keys(source).sort()) {
    if (!ALLOWED_KEYS.has(key) || HIDDEN_KEYS.test(key)) continue;
    const scalar = safeScalar(source[key]);
    if (scalar !== undefined) output[key] = scalar;
  }
  if (typeof output.id === "string") safeId(output.id);
  if (typeof output.contentHash === "string" && !/^[a-f0-9]{64}$/u.test(output.contentHash)) delete output.contentHash;
  return Object.freeze(output) as SafeInspectRecord;
}

/** Bounded deterministic operator inspection. */
export async function inspectRecords(options: InspectOptions = {}, source: InspectRecordSource = {}): Promise<InspectResult> {
  const limit = boundedLimit(options.limit);
  const ids = options.ids?.map((value) => safeId(value));
  const recordTypes = options.recordTypes?.map((value) => safeId(value));
  const query: InspectOptions = { limit, ...(ids === undefined ? {} : { ids: [...new Set(ids)] }), ...(recordTypes === undefined ? {} : { recordTypes: [...new Set(recordTypes)] }) };
  const raw = source.read === undefined ? source.records ?? [] : await source.read(query);
  if (!Array.isArray(raw)) throw new Error("Inspect source returned invalid records");
  const records: SafeInspectRecord[] = [];
  for (const value of raw) {
    if (records.length >= limit) break;
    let projected: SafeInspectRecord;
    try { projected = redactInspectRecord(value); } catch { continue; }
    if (ids !== undefined && (projected.id === undefined || !ids.includes(projected.id))) continue;
    if (recordTypes !== undefined && (projected.recordType === undefined || !recordTypes.includes(projected.recordType))) continue;
...
const INSPECT_TYPES: readonly PointRecordType[] = ["episode", "curated_memory", "curated_current", "raptor_summary", "job", "lease", "proposal", "coverage", "evidence_link", "tombstone"];
function wireMetadata(id: string, payload: Record<string, unknown>): Record<string, unknown> {
...
export async function inspectQdrantRecords(config: RuntimeConfig, options: InspectOptions = {}, fetchImpl: typeof fetch = globalThis.fetch): Promise<InspectResult> {
  const limit = boundedLimit(options.limit);
  const requestedIds = options.ids?.map((value) => safeId(value));
  const requestedTypes = options.recordTypes === undefined ? [...INSPECT_TYPES] : options.recordTypes.map((value) => safeId(value) as PointRecordType);
  if (requestedTypes.length === 0 || requestedTypes.length > INSPECT_TYPES.length || requestedTypes.some((value) => !INSPECT_TYPES.includes(value))) throw new TypeError("Inspect record types are invalid");
  ...
  for (let page = 0; page < 4 && rows.length < limit; page += 1) {
    ...
    for (const point of result.points) {
      ...
      if (requestedIds !== undefined && !requestedIds.includes(logicalId) && !requestedIds.includes(point.id)) continue;
      rows.push(projected);
