...
export class BoundIngestRuntime {
  ...
  constructor(input: BoundIngestRuntimeInput, issuer: symbol) {
    if (issuer !== INGEST_RUNTIME_ISSUER) throw new TypeError("Ingest runtime requires the module issuer");
    ...
    if (!ProductionCoordinationStore.isValid(store)) throw new TypeError("Ingest runtime requires the branded production store");
    if (!BoundQdrantDestination.isValid(qdrant)) throw new TypeError("Ingest runtime requires a bound Qdrant destination");
    if (!BoundEmbeddingDestination.isValid(embedding)) throw new TypeError("Ingest runtime requires a bound embedding destination");
    ...
    if (storeTransport !== qdrantTransport) throw new TypeError("Ingest runtime store and Qdrant destination must share the exact writer transport");
    if (storeEndpoint !== qdrantEndpoint || storeOwnerHost !== qdrantOwnerHost || storeCollection !== qdrantCollection) throw new TypeError("Ingest runtime store and Qdrant destination identity mismatch");
    if (embeddingPolicyHash !== qdrantPolicyHash || embeddingPolicyEpoch !== qdrantPolicyEpoch) throw new TypeError("Ingest runtime coordination identity mismatch");
    this.#issuer = issuer;
    this.store = store;
    this.qdrant = qdrant;
...
export function bindIngestRuntime(input: BoundIngestRuntimeInput): BoundIngestRuntime {
