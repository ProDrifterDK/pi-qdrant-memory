...
export function canonicalizeEmbeddingVector(value: unknown, dimension: number): number[] {
  if (!Number.isSafeInteger(dimension) || dimension < 1 || !Array.isArray(value) || value.length !== dimension || !value.every(component => typeof component === "number" && Number.isFinite(component))) throw invalidResponse("Embedding vector has invalid components");
  let scale = 0;
  let sum = 1;
  for (const component of value) {
    const absolute = Math.abs(component);
    if (absolute === 0) continue;
    if (scale < absolute) { const ratio = scale / absolute; sum = 1 + sum * ratio * ratio; scale = absolute; }
    else { const ratio = absolute / scale; sum += ratio * ratio; }
  }
  if (scale === 0 || !Number.isFinite(sum)) throw invalidResponse("Embedding vector has zero or invalid norm");
  const scaledNorm = Math.sqrt(sum);
  const canonical = value.map(component => Math.fround((component / scale) / scaledNorm));
  if (canonical.every(component => component === 0)) throw invalidResponse("Embedding vector collapsed during canonicalization");
  return canonical;
}
...
function snapshotEmbeddingVector(value: unknown): readonly number[] {
  if (!Array.isArray(value) || nodeTypes.isProxy(value) || Object.getPrototypeOf(value) !== Array.prototype || Object.getOwnPropertySymbols(value).length > 0) throw new Error("Embedding vector must be a dense plain array");
  const names = Object.getOwnPropertyNames(value);
  if (value.length !== 1024 || names.length !== 1025 || !names.includes("length")) throw new Error("Embedding vector must have 1024 finite components");
  const snapshot: number[] = [];
  for (let index = 0; index < 1024; index += 1) {
    const descriptor = Object.getOwnPropertyDescriptor(value, String(index));
    if (descriptor === undefined || !("value" in descriptor) || !descriptor.enumerable || typeof descriptor.value !== "number" || !Number.isFinite(descriptor.value)) throw new Error("Embedding vector must have 1024 finite components");
