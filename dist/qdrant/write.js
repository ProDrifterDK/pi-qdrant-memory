...
export class ProductionCoordinationStore {
     ...
     */
    async activateWorkerPolicyControl(workerPolicyId) {
        if (typeof workerPolicyId !== "string" || !/^[a-f0-9]{64}$/u.test(workerPolicyId))
            return false;
        let current;
        try {
            current = await this.readControl();
        ...
        if (current.version !== 0 || current.state !== "active")
            return false;
        try {
            assertBootstrapControl(current, this.ownerHost);
        }
        catch {
            return false;
        }
        let policies;
        try {
            policies = await this.readProcessingPolicies([workerPolicyId]);
        }
        catch {
            return false;
        }
        if (policies.length !== 1 || policies[0].id !== workerPolicyId || policies[0].policy.id !== workerPolicyId)
            return false;
        ...
        const next = { ...pending, contentHash: canonicalRecordHash(pending) };
        if (!(await this.#protocol.compareAndSwapControl(0, next)))
            return false;
        try {
            return await this.readControl();
        }
        catch {
            return false;
        }
