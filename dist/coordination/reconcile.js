...
function validateSlice(slice, batchSize, cursor, ownerHost) {
    if (slice === null || typeof slice !== "object" || !Array.isArray(slice.episodes) || slice.episodes.length > batchSize)
        throw new TypeError("Reconcile slice episodes are invalid");
    if (slice.nextOffset !== undefined && (typeof slice.nextOffset !== "string" || slice.nextOffset.length === 0 || slice.nextOffset.length > 512))
        throw new TypeError("Reconcile slice cursor is invalid");
    ...
    for (const episode of slice.episodes) {
        if (episode === null || typeof episode !== "object" || episode.recordType !== "episode" || !isPersistedMemoryRecord(episode, { vectorDimension: 1024 }))
            throw new TypeError("Reconcile slice contains an invalid episode");
        ...
        if (episode.ownerHost !== ownerHost)
            throw new TypeError("Reconcile slice contains a foreign episode");
        if (episode.id <= previous || pageIds.has(episode.id))
            throw new TypeError("Reconcile slice episodes must be strictly sorted and unique within the page");
    ...
    if (lastId !== undefined && slice.nextOffset !== undefined && slice.nextOffset !== lastId)
        throw new TypeError("Reconcile slice cursor must be the last processed episode id");
    ...
    if (slice.nextOffset !== undefined && cursor !== undefined && !(slice.nextOffset > cursor))
        throw new TypeError("Reconcile slice cursor must advance beyond the input cursor");
    return [...slice.episodes];
}
/** Scan episodes in bounded slices, batch-retrieve their coverage IDs, enqueue missing/late episodes. */
export async function findMissingEpisodes(input) {
    ...
    if (!ProductionCoordinationStore.isValid(store))
        throw new TypeError("Coverage scan requires a genuine production store");
    ...
    if (!Number.isSafeInteger(batchSize) || batchSize < 1 || batchSize > 1024)
        throw new TypeError("Reconcile batch size is invalid");
    if (!Number.isSafeInteger(maxSlices) || maxSlices < 1 || maxSlices > 1024)
        throw new TypeError("Reconcile slice bound is invalid");
    if (!Number.isSafeInteger(maxMissing) || maxMissing < 1 || maxMissing > 1024)
        throw new TypeError("Reconcile missing bound is invalid");
    if (typeof listEpisodes !== "function")
        throw new TypeError("Reconcile requires an episode scanner");
    if (typeof extractorRevision !== "string" || extractorRevision.length === 0 || extractorRevision.length > 512)
        throw new TypeError("Reconcile extractor revision is invalid");
    if (!Number.isSafeInteger(policyEpoch) || policyEpoch < 0 || !Number.isSafeInteger(privacyEpoch) || privacyEpoch < 0)
        throw new TypeError("Reconcile policy/privacy epochs are invalid");
    if (typeof policyHash !== "string" || policyHash.length === 0 || policyHash.length > 512 || typeof policyIntersectionId !== "string" || policyIntersectionId.length === 0 || policyIntersectionId.length > 512)
        throw new TypeError("Reconcile policy identity is invalid");
    if (offset !== undefined && (typeof offset !== "string" || offset.length === 0 || offset.length > 512))
        throw new TypeError("Reconcile cursor is invalid");
    let cursor = offset;
    const missing = [];
    let scanned = 0;
    ...
    for (let sliceIndex = 0; sliceIndex < maxSlices; sliceIndex += 1) {
        if (signal?.aborted)
            throw new TypeError("Reconcile scan aborted");
        let slice;
        try {
            slice = await listEpisodes(cursor, batchSize);
        }
        catch {
            throw new TypeError("Reconcile episode scan failed");
        }
        const episodes = validateSlice(slice, batchSize, cursor, store.ownerHost);
        if (episodes.length === 0) {
            if (slice.nextOffset !== undefined)
                throw new TypeError("Reconcile empty slice cannot carry a resume cursor");
        ...
        for (const episode of episodes) {
            if (processed.has(episode.id))
                continue; // overlap duplicate within this call
            ...
            if (!overlap && (lastForward === undefined || episode.id > lastForward))
                lastForward = episode.id;
            if (!covered.has(episode.id))
 ...
 */
export async function reconcileCoverage(input) {
    ...
    const store = input.store;
    if (!ProductionCoordinationStore.isValid(store))
        throw new TypeError("Coverage reconcile requires a genuine production store");
    const listEpisodes = input.listEpisodes;
    const extractorRevision = input.extractorRevision;
    const policyEpoch = input.policyEpoch;
    const policyHash = input.policyHash;
    ...
    const offset = input.offset;
    const signal = input.signal;
    const snapshot = { store, listEpisodes, extractorRevision, policyEpoch, policyHash, policyIntersectionId, privacyEpoch, maxSlices: 1 };
    if (batchSize !== undefined)
        snapshot.batchSize = batchSize;
    if (maxMissing !== undefined)
        snapshot.maxMissing = maxMissing;
    if (offset !== undefined)
