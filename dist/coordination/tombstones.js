...
import { isContentTarget, isOccurrenceTarget, isStateTarget, isTombstoneTarget, tombstoneId } from "../domain/ids.js";
 ...
 */
export async function createTombstone(store, input) {
    if (!ProductionCoordinationStore.isValid(store))
        throw new TypeError("Tombstone creation requires a genuine production store");
    return store.createTombstone(input);
}
export async function readTombstones(store, targetIds) {
    if (!ProductionCoordinationStore.isValid(store))
        throw new TypeError("Tombstone read requires a genuine production store");
    if (!Array.isArray(targetIds) || targetIds.length === 0 || targetIds.length > 1024 || targetIds.some((id) => typeof id !== "string" || id.length === 0 || id.length > 512))
...
export function tombstoneTargets(record) {
    const targets = [];
    switch (record.recordType) {
        ...
        case "curated_memory":
            ...
            if (record.primaryEvidenceEpisodeId !== undefined)
                targets.push({ scope: "occurrence", targetId: record.primaryEvidenceEpisodeId });
            for (const sourceId of record.sourceEpisodeIds ?? [])
                targets.push({ scope: "occurrence", targetId: sourceId });
            for (const sourceId of record.provenance ?? [])
                targets.push({ scope: "occurrence", targetId: sourceId });
            break;
 ...
 */
export async function isVisibleAfterTombstoneCheck(store, record) {
    ...
    if (record.recordType === "curated_memory" || record.recordType === "curated_current" || record.recordType === "raptor_summary") {
        ...
        if (!directSourceIds.every((id) => EPISODE_UUID_RE.test(id)))
            return false;
    ...
    for (const target of targets) {
        if (!isTombstoneTarget(target.scope, target.targetId))
            return false;
        if (target.scope === "occurrence" && !isOccurrenceTarget(target.targetId))
            return false;
    }
    ...
    try {
        if (episodeTargets.length > 0) {
            ...
            for (const targetId of episodeTargets) {
                ...
                if (episode === undefined || episode.ownerHost !== record.ownerHost)
                    return false;
            }
        }
        const tombstones = await store.readTombstones(ids);
        for (const target of targets)
            if (tombstoned(tombstones, target))
                return false;
        return true;
    }
    catch {
...
function tombstoned(tombstones, target) {
    for (const tombstone of tombstones) {
        if (tombstone.scope !== target.scope || tombstone.targetId !== target.targetId)
            continue;
 ...
 */
export class BoundIngestTombstoneReader {
    #issuer;
    #store;
    #ownerHost;
    /** Public constructor is unusable without the module-private issuer symbol. */
    constructor(store, ownerHost, issuer) {
        if (issuer !== INGEST_TOMBSTONE_READER_ISSUER)
            throw new TypeError("Tombstone reader requires the module issuer");
        if (ownerHost !== "pi" && ownerHost !== "prime")
            throw new TypeError("Tombstone reader owner is invalid");
        if (!ProductionCoordinationStore.isValid(store) || store.ownerHost !== ownerHost)
            throw new TypeError("Tombstone reader requires the branded production store of its owner");
        this.#issuer = issuer;
        this.#store = store;
    ...
    get ownerHost() { return this.#ownerHost; }
    async readTombstoned(episodeIds) {
        if (!Array.isArray(episodeIds) || episodeIds.length === 0 || episodeIds.length > 1024 || episodeIds.some((id) => typeof id !== "string" || id.length === 0 || id.length > 512))
            throw new TypeError("Tombstone episode IDs are invalid");
...
export function createIngestTombstoneReader(store, ownerHost) {
