...
const EMBEDDINGS_TOKEN = new WeakMap();
export class EmbeddingsClient {
    ...
    constructor(options) {
        ...
        const snapshotOptions = { baseUrl, model, dimension, queryPrefix, timeoutMs };
        if (apiKey !== undefined)
            snapshotOptions.apiKey = apiKey;
        if (fetchImpl !== undefined)
            snapshotOptions.fetchImpl = fetchImpl;
        const endpoint = canonicalEgressEndpoint(snapshotOptions.baseUrl);
        if (typeof snapshotOptions.model !== "string" || snapshotOptions.model.length === 0 || !Number.isInteger(snapshotOptions.dimension) || snapshotOptions.dimension <= 0 || typeof snapshotOptions.queryPrefix !== "string" || !Number.isFinite(snapshotOptions.timeoutMs) || snapshotOptions.timeoutMs <= 0)
        ...
        this.#issuer = EMBEDDINGS_CLIENT_ISSUER;
        Object.freeze(this);
    }
    /** Real-brand check: only genuine EmbeddingsClient instances pass; forged prototypes and structural objects fail. */
    static isValid(value) {
        if (typeof value !== "object" || value === null || !(#issuer in value) || !EMBEDDINGS_STATE.has(value))
            return false;
    ...
    static isProductionBound(value) {
        if (!EmbeddingsClient.isValid(value))
            return false;
        const state = EMBEDDINGS_STATE.get(value);
        return state !== undefined && state.injectedFetch === false;
    }
    /** Opaque per-instance frozen transport identity (empty object; never options). */
    get transport() { const token = EMBEDDINGS_TOKEN.get(this); if (token === undefined)
        throw new TypeError("Embedding transport token is missing"); return token; }
    async request(input, model, signal) {
        if (!Number.isInteger(EMBEDDINGS_STATE.get(this).options.dimension) || EMBEDDINGS_STATE.get(this).options.dimension <= 0)
            throw new MemoryClientError("configuration", "Embedding dimension must be a positive integer");
        const headers = { "content-type": "application/json" };
        if (EMBEDDINGS_STATE.get(this).options.apiKey !== undefined)
            headers.authorization = `Bearer ${EMBEDDINGS_STATE.get(this).options.apiKey}`;
        const transportOptions = { timeoutMs: EMBEDDINGS_STATE.get(this).options.timeoutMs };
        if (signal !== undefined)
            transportOptions.signal = signal;
        if (EMBEDDINGS_STATE.get(this).fetchImpl !== undefined)
            transportOptions.fetchImpl = EMBEDDINGS_STATE.get(this).fetchImpl;
        const result = await fetchJson(`${EMBEDDINGS_STATE.get(this).options.baseUrl.replace(/\/+$/, "")}/embeddings`, { method: "POST", headers, body: JSON.stringify({ model, input }) }, transportOptions);
    ...
    async embedDocument(input) {
        if (input.model !== "bge-m3" || EMBEDDINGS_STATE.get(this).options.model !== "bge-m3" || EMBEDDINGS_STATE.get(this).options.dimension !== 1024)
...
export class ValidatedEmbeddingDocumentClient {
    ...
    constructor(endpoint, client, issuer) {
        if (issuer !== VALIDATED_EMBEDDING_ISSUER)
            throw new TypeError("Embedding client requires the module issuer");
        this.#issuer = issuer;
        this.endpoint = endpoint;
        this.#embedDocument = client.embedDocument.bind(client);
        Object.freeze(this);
    }
    ...
    static bind(input) {
        ...
        if (!EmbeddingsClient.isProductionBound(client) || canonicalEgressEndpoint(client.endpoint) !== endpoint)
            throw new TypeError("Embedding document client endpoint pairing is invalid");
        if (!Object.isFrozen(client))
...
export class BoundEmbeddingDestination {
    ...
    constructor(input, issuer) {
        if (issuer !== BOUND_EMBEDDING_DESTINATION_ISSUER)
            throw new TypeError("Embedding destination requires the module issuer");
        this.#issuer = issuer;
    ...
    embed(input) { return this.#embed(input); }
}
...
export function createEmbeddingDestinationFactory(input) {
    ...
    const endpoint = canonicalEgressEndpoint(input.endpoint);
    if (!ValidatedEmbeddingDocumentClient.isValid(client) || client.endpoint !== endpoint)
        throw new TypeError("Embedding document client endpoint pairing is invalid");
    if (typeof coordinationPolicyHash !== "string" || coordinationPolicyHash.length === 0 || coordinationPolicyHash.length > 512 || !Number.isSafeInteger(coordinationPolicyEpoch) || coordinationPolicyEpoch < 0)
        throw new TypeError("Embedding coordination binding is invalid");
