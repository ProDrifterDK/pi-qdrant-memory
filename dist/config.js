...
function boundedIdentifier(name, raw, fallback, max, pattern = /^[A-Za-z0-9._:/-]+$/u) {
...
function boundedText(name, raw, fallback, max) {
    const value = stringValue(name, raw, fallback);
    if (value.length > max || /(?:api[-_]?key|access[-_]?token|authorization|bearer|credential|password|secret|token)/iu.test(value))
        throw new Error(`${name} must be bounded and redacted`);
    return value;
}
...
function normalizeUrl(name, raw, fallback) {
    ...
    if (parsed.protocol !== "http:" && parsed.protocol !== "https:")
        throw new Error(`${name} must use REST-safe http(s)`);
...
function destinations(name, raw, fallback) {
    ...
    return raw.map((value, index) => {
        ...
        if (seen.has(key))
            throw new Error(`${name} contains a duplicate destination`);
        seen.add(key);
        return destination;
    });
}
...
function explicitProperty(record, key) {
    return Object.prototype.hasOwnProperty.call(record, key);
}
function resolvedConfiguredQdrantSections(file) {
    ...
    for (const host of ["pi", "prime"]) {
        const hostValue = file[host];
        if (!isRecord(hostValue) || !isRecord(hostValue.qdrant))
            continue;
...
function rejectDuplicateConfiguredDestinations(file) {
    const seen = new Map();
    for (const item of resolvedConfiguredQdrantSections(file)) {
        const key = `${item.url}\0${item.collection}`;
        const previous = seen.get(key);
        if (previous !== undefined)
            throw new Error(`Qdrant endpoint/collection is configured by both ${previous} and ${item.name}`);
        seen.set(key, item.name);
    }
}
function effectiveQdrantForHost(file, host) {
...
export async function loadConfig(host, deps) {
    ...
    const result = {
        ...
        projects: {
            registrations: isRecord(projects.registrations) ? Object.fromEntries(Object.entries(projects.registrations).map(([key, value]) => {
                ...
                if (!isAbsolute(canonicalPath) || canonicalPath.length > 4096)
                    throw new Error(`projects.registrations.${key}.canonicalPath must be an absolute bounded path`);
                if (fingerprint.length > 512 || /(?:api[-_]?key|token|secret|password)/iu.test(fingerprint) || alias.length > 256 || /(?:api[-_]?key|token|secret|password)/iu.test(alias) || alias !== key || !/^[A-Za-z0-9._-]+$/u.test(alias))
                    throw new Error(`projects.registrations.${key} contains a secret, overlap, or unbounded identity`);
        ...
        coordination: {
            ...
            readConsistency: (() => {
                ...
                if (replication > 1 && configured === 1) {
                    if (readConsistencyExplicit)
                        throw new Error("coordination.readConsistency must be at least majority");
                    return "majority";
                }
                if (replication > 1 && typeof configured === "number" && configured < Math.ceil((replication + 1) / 2))
                    throw new Error("coordination.readConsistency must be at least majority");
                return configured;
            })(),
    ...
    for (let index = 0; index < registrations.length; index += 1)
        for (let other = index + 1; other < registrations.length; other += 1) {
            const left = registrations[index].canonicalPath.replace(/\/+$/u, "");
            const right = registrations[other].canonicalPath.replace(/\/+$/u, "");
            if (left === right || left.startsWith(`${right}/`) || right.startsWith(`${left}/`))
                throw new Error("Project registration path overlap is ambiguous");
        }
    if (result.capture.enabled && !captureRetentionExplicit)
        throw new Error("capture.episodeRetentionDays must be explicitly selected before capture is enabled");
    if (result.capture.enabled && !captureEgressExplicit)
        throw new Error("privacy.egressMode must be explicitly selected before capture is enabled");
    const effectivePi = effectiveQdrantForHost(file, "pi");
    const effectivePrime = effectiveQdrantForHost(file, "prime");
    ...
    if (activePi.enabled && activePrime.enabled && activePi.url === activePrime.url && activePi.collection === activePrime.collection)
        throw new Error("Effective Pi and Prime endpoint/collection collision");
    if (result.outbox.retryBaseMs > result.outbox.retryMaxMs)
        throw new Error("outbox.retryBaseMs must not exceed retryMaxMs");
    if (result.qdrant.replicationFactor > 1 && result.qdrant.writeConsistencyFactor < Math.ceil((result.qdrant.replicationFactor + 1) / 2))
        throw new Error("qdrant.writeConsistencyFactor must satisfy replication majority");
    if (result.outbox.sharedFilesystem && result.outbox.nodeId === undefined)
        throw new Error("outbox.nodeId is required for a shared filesystem");
    if (result.privacy.egressMode === "allowlist" && (result.privacy.allowedQdrantDestinations.length === 0 || result.privacy.allowedEmbeddingDestinations.length === 0))
        throw new Error("allowlist egress requires explicit Qdrant and embedding destinations");
