...
function isSafeOpaqueFormat(value) {
...
function isHighEntropy(value) {
    if (value.startsWith("[") && value.endsWith("]"))
        return false;
    if (/^(?:redacted|placeholder|removed|empty)$/iu.test(value))
        return false;
...
function labeledSha256(value, start) {
...
export function scanFinalText(value) {
    try {
        ...
        for (const pattern of SECRET_PATTERNS)
            if (pattern.test(markerSafe))
                return "rejected";
        STANDALONE_BEARER.lastIndex = 0;
        if (STANDALONE_BEARER.test(markerSafe))
            return "rejected";
        if (/\b[A-Za-z][A-Za-z0-9_.-]{1,80}\s*(?::|=)\s*(?=$|[\r\n])/gu.test(markerSafe)) {
            const emptyAssignment = /\b([A-Za-z][A-Za-z0-9_.-]{1,80})\s*(?::|=)\s*(?=$|[\r\n])/u.exec(markerSafe);
            if (emptyAssignment !== null && sensitiveName(emptyAssignment[1] ?? ""))
                return "rejected";
        }
        ...
        for (let match = ASSIGNMENT.exec(markerSafe); match !== null; match = ASSIGNMENT.exec(markerSafe)) {
            ...
            if (sensitiveName(name)) {
                ...
                try {
                    normalized = decodeURIComponent(assigned);
                }
                catch {
                    return "rejected";
                }
                if (!PLACEHOLDER.test(normalized.trim()))
                    return "rejected";
            }
        }
        ...
        for (let match = QUOTED_ASSIGNMENT.exec(markerSafe); match !== null; match = QUOTED_ASSIGNMENT.exec(markerSafe)) {
            if (sensitiveName(match[1] ?? "") && !PLACEHOLDER.test((match[2] ?? "").trim()))
                return "rejected";
        }
        ...
        for (let match = HIGH_ENTROPY_CANDIDATE.exec(markerSafe); match !== null; match = HIGH_ENTROPY_CANDIDATE.exec(markerSafe)) {
            if (!isSafeOpaqueFormat(match[0]) && isHighEntropy(match[0]) && !(match[0].length === 64 && labeledSha256(markerSafe, match.index)))
                return "rejected";
        }
        return "passed";
    }
