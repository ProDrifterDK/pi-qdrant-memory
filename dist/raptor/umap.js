...
function ownDenseArray(value, label, max) {
...
function denseMatrix(input) {
    const rows = ownDenseArray(input, "UMAP input", 65_536);
    if (rows.length === 0)
        return [];
    const first = rows[0];
    if (!Array.isArray(first) || nodeTypes.isProxy(first))
        throw new TypeError("UMAP row is invalid");
    const dimension = first.length;
    if (!Number.isSafeInteger(dimension) || dimension < 1 || dimension > 4096)
        throw new TypeError("UMAP dimension is invalid");
    return rows.map((candidate) => { if (!Array.isArray(candidate))
        throw new TypeError("UMAP row is invalid"); const row = ownDenseArray(candidate, "UMAP row", 4096); if (row.length !== dimension || row.some((value) => typeof value !== "number" || !Number.isFinite(value)))
        throw new TypeError("UMAP input must be a dense finite matrix"); return row; });
}
function boundedOptions(count, inputDimension, input) {
    if (input.scope !== "global" && input.scope !== "local")
        throw new TypeError("UMAP scope is invalid");
    if (!Number.isSafeInteger(input.dimensions) || input.dimensions < 1 || input.dimensions > 64)
        throw new TypeError("UMAP dimensions must be in 1..64");
    if (!Number.isSafeInteger(input.neighbors) || input.neighbors < 1 || input.neighbors > 65_536)
        throw new TypeError("UMAP neighbors are invalid");
    ...
    if (seed.length === 0 || seed.length > 4096)
        throw new TypeError("UMAP seed is invalid");
    return Object.freeze({ scope: input.scope, seed, nComponents, nNeighbors });
}
function removeZeroVariance(matrix) {
    if (matrix.length === 0)
        return { matrix, allEqual: true };
    ...
    if (active.length === 0)
        return { matrix: matrix.map(() => [0]), allEqual: true };
    return { matrix: matrix.map((row) => active.map((column) => row[column])), allEqual: false };
}
export function reduceUmapDetailed(input, options) {
    const owned = denseMatrix(input);
    const parameters = boundedOptions(owned.length, owned[0]?.length ?? 1, options);
    if (owned.length === 0)
        return Object.freeze({ embedding: Object.freeze([]), parameters });
    if (owned.length === 1)
        return Object.freeze({ embedding: Object.freeze([Object.freeze([0])]), parameters });
    const normalized = removeZeroVariance(owned);
    if (normalized.allEqual)
        return Object.freeze({ embedding: Object.freeze(owned.map(() => Object.freeze(Array.from({ length: parameters.nComponents }, () => 0)))), parameters });
    if (owned.length === 2) {
    ...
    if (!Array.isArray(result) || result.length !== owned.length || result.some((row) => !Array.isArray(row) || row.length !== parameters.nComponents || row.some((value) => typeof value !== "number" || !Number.isFinite(value))))
        throw new TypeError("UMAP produced an invalid embedding");
    return Object.freeze({ embedding: Object.freeze(result.map((row) => Object.freeze([...row]))), parameters });
}
export function reduceUmap(input, options) { return reduceUmapDetailed(input, options).embedding; }
export function reduceUmapPair(vectors, input) {
