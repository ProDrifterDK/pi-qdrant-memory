...
export function seedWords(seed) {
    if ((typeof seed !== "string" && typeof seed !== "number") || (typeof seed === "number" && !Number.isFinite(seed)))
        throw new TypeError("RAPTOR seed must be a bounded string or finite number");
    const text = String(seed);
    if (text.length === 0 || text.length > 4096)
        throw new TypeError("RAPTOR seed must be bounded");
    const digest = createHash("sha256").update(text, "utf8").digest();
    const words = [digest.readUInt32BE(0), digest.readUInt32BE(4), digest.readUInt32BE(8), digest.readUInt32BE(12)];
    if (words.every((word) => word === 0))
        words[0] = NON_ZERO_FALLBACK;
    return Object.freeze(words);
}
/** Deterministic xoshiro128** PRNG with unbiased bounded integers. */
export class Xoshiro128StarStar {
    #state;
    constructor(seed) { this.#state = [...seedWords(seed)]; }
    nextUint32() {
    ...
    nextInt(maxExclusive) {
        if (!Number.isSafeInteger(maxExclusive) || maxExclusive < 1 || maxExclusive > UINT32_RANGE)
            throw new TypeError("PRNG maxExclusive must be in 1..2^32");
