...
import { initializeDestination, validateInitializationDisclosure } from "./init.js";
import { memoryStatus } from "./status.js";
import { projectStatusCommand, registerProjectCommand, unregisterProjectCommand } from "./project.js";
import { planPrivacyRevoke, revokePrivacy } from "./privacy.js";
import { planForget, runForget as applyForgetPlan } from "./forget.js";
import { inspectQdrantRecords } from "./inspect.js";
...
const COMMANDS = ["init", "project", "privacy", "status", "curate", "raptor", "reconcile", "inspect", "forget"];
const OPERATION_COMMANDS = ["curate", "raptor", "reconcile"];
...
export class CliInputError extends Error {
    constructor(message = "invalid arguments or configuration") { super(message); this.name = "CliInputError"; }
}
export class CliConfigError extends Error {
    constructor(message = "invalid arguments or configuration") { super(message); this.name = "CliConfigError"; }
}
export class CliApprovalError extends CliInputError {
    constructor(message = "approval does not match plan") { super(message); this.name = "CliApprovalError"; }
}
function projectDeps(home, env, readTextFile, writeTextFile) {
    const gitTopLevel = (cwd) => new Promise((resolve, reject) => execFile("git", ["-C", cwd, "rev-parse", "--show-toplevel"], { encoding: "utf8", shell: false }, (error, stdout) => error === null ? resolve(String(stdout).trim()) : reject(error)));
    const canonicalize = async (path) => realpathSync(path);
...
export function defaultCliDependencies(options = {}) {
    ...
    const writeTextFile = options.writeTextFile ?? (async (path, text) => {
    ...
    const planStorage = (config) => options.writeTextFile === undefined
        ? createStoredPlan(config)
        : createStoredPlan(config, { readTextFile, writeTextFile });
    const assertAdmin = () => { if (loadAdminProcessSecrets(env).destinationApiKey === undefined)
        throw new CliConfigError("Human Qdrant admin key is required for this operation"); };
    const reconcile = async (config) => { assertAdmin(); await productionOperation(config, env, { command: "reconcile", action: "enqueue" }); };
    return {
        ...
        initialize: config => {
            ...
            if (adminKey === undefined)
                throw new CliConfigError("Human Qdrant admin key is required for CLI initialization");
        ...
        privacyLoadPlan: async (config, planId) => planStorage(config).load("privacy", planId),
        ...
        forgetLoadPlan: async (config, planId) => planStorage(config).load("forget", planId),
...
function parseWithOptions(args, options, allowPositionals = false) {
    try {
...
function baseOptions() { return { json: { type: "boolean" }, help: { type: "boolean", short: "h" } }; }
function base(values) { return { json: values.json === true, help: values.help === true }; }
function explicitHost(env) { const value = env[ADMIN_HOST_ENVIRONMENT]; if (value !== "prime" && value !== "pi")
    throw new CliInputError(); return value; }
function requestedJson(args) { return args.includes("--json"); }
function topLevelHelpRequest(args) { return args.length > 0 && args.every(value => value === "--help" || value === "-h" || value === "--json") && args.some(value => value === "--help" || value === "-h"); }
async function configured(host, deps) { try {
    return await deps.loadConfig(host);
}
catch {
    throw new CliConfigError();
} }
function safeOutputValue(value, key = "") {
    if (/(?:api[-_]?key|access[-_]?token|authorization|bearer|credential|password|secret|token)/iu.test(key))
        return "[redacted]";
    if (/(?:^|_)(?:payload|vector|prompt|completion|tool_args|tool_result|raw)$/iu.test(key))
        return "[redacted]";
    if (Array.isArray(value))
        return value.slice(0, 1024).map(item => safeOutputValue(item));
    if (typeof value === "object" && value !== null)
        return Object.fromEntries(Object.entries(value).sort(([a], [b]) => a.localeCompare(b)).map(([childKey, child]) => [childKey, safeOutputValue(child, childKey)]));
    if (typeof value === "string" && value.length > 16000)
        return `${value.slice(0, 16000)}…`;
    return value;
}
function output(deps, json, command, value) { const safe = safeOutputValue(value); deps.writeStdout(json ? `${JSON.stringify({ command, ...safe })}\n` : `${command}\n${JSON.stringify(safe, null, 2)}\n`); }
function help(deps, json, usage) { deps.writeStdout(json ? `${JSON.stringify({ usage })}\n` : `${usage}\n`); }
function requireString(value, name) { if (typeof value !== "string" || value.length === 0 || value.length > 512 || /[\u0000-\u001f\u007f-\u009f]/u.test(value))
    throw new CliInputError(`invalid ${name}`); return value; }
function positiveLimit(value) { if (value === undefined)
    return 64; const parsed = Number(value); if (!Number.isSafeInteger(parsed) || parsed < 1 || parsed > 256)
    throw new CliInputError(); return parsed; }
function retention(value) { if (value === "indefinite")
    return value; const parsed = Number(value); if (!Number.isSafeInteger(parsed) || parsed < 1 || parsed > 3650)
    throw new CliInputError(); return parsed; }
async function runInit(args, deps, config) {
    const parsed = parseWithOptions(args, { ...baseOptions(), retention: { type: "string" }, egress: { type: "string" }, confirm: { type: "boolean" } });
    const b = base(parsed.values);
    if (b.help) {
        help(deps, b.json, COMMAND_HELP.init);
        return 0;
    }
    const disclosure = parsed.values.retention === undefined && parsed.values.egress === undefined && parsed.values.confirm !== true ? undefined : { retention: retention(parsed.values.retention ?? config.capture.episodeRetentionDays), egressMode: parsed.values.egress === "local_only" || parsed.values.egress === "allowlist" ? parsed.values.egress : parsed.values.egress === undefined ? config.privacy.egressMode : (() => { throw new CliInputError(); })(), confirmed: parsed.values.confirm === true };
    try {
    ...
    output(deps, b.json, "init", result);
    return 0;
}
async function runProject(args, deps, config) {
    const parsed = parseWithOptions(args, { ...baseOptions(), path: { type: "string" }, alias: { type: "string" }, confirm: { type: "boolean" } }, true);
    const b = base(parsed.values);
    if (b.help) {
        help(deps, b.json, COMMAND_HELP.project);
        return 0;
    }
    const subcommand = parsed.positionals[0];
    if (subcommand === "register") {
        if (parsed.positionals.length !== 1 || deps.projectRegister === undefined || parsed.values.path === undefined || parsed.values.alias === undefined)
            throw new CliInputError();
        output(deps, b.json, "project", await deps.projectRegister({ path: requireString(parsed.values.path, "path"), alias: requireString(parsed.values.alias, "alias"), confirmed: parsed.values.confirm === true }, config));
        return 0;
    }
    if (subcommand === "unregister") {
        if (parsed.positionals.length !== 1 || deps.projectUnregister === undefined || parsed.values.alias === undefined)
            throw new CliInputError();
        output(deps, b.json, "project", await deps.projectUnregister({ alias: requireString(parsed.values.alias, "alias"), confirmed: parsed.values.confirm === true }, config));
        return 0;
    }
    if (subcommand === "status") {
        if (parsed.positionals.length !== 1 || deps.projectStatus === undefined)
            throw new CliInputError();
        output(deps, b.json, "project", await deps.projectStatus(requireString(parsed.values.path ?? deps.cwd ?? process.cwd(), "path"), config));
        return 0;
    }
    throw new CliInputError();
}
async function runPrivacy(args, deps, config) {
    const normalizedArgs = [...args];
    const inlinePlanIndex = normalizedArgs.findIndex(argument => argument.startsWith("--plan="));
    if (inlinePlanIndex >= 0)
        normalizedArgs[inlinePlanIndex] = `--plan-id=${normalizedArgs[inlinePlanIndex].slice("--plan=".length)}`;
    ...
    const b = base(parsed.values);
    if (b.help) {
        help(deps, b.json, COMMAND_HELP.privacy);
        return 0;
    }
    if (parsed.positionals.length !== 1 || parsed.positionals[0] !== "revoke" || deps.privacyPlan === undefined)
        throw new CliInputError();
    const suppliedPlan = parsed.values["plan-id"] === undefined ? undefined : requireString(parsed.values["plan-id"], "plan");
    const destinationValues = [parsed.values.destination, parsed.values.destinations].filter((value) => typeof value === "string").flatMap(value => value.split(",").map(item => item.trim()).filter(Boolean));
    ...
    if (suppliedPlan !== undefined && suppliedPlan !== plan.id)
        throw new CliApprovalError("privacy plan does not match current state");
    if (parsed.values.approve === undefined) {
        output(deps, b.json, "privacy", plan);
        return 0;
    }
    if (deps.privacyApply === undefined)
        throw new CliInputError();
    output(deps, b.json, "privacy", await deps.privacyApply(config, { plan, approvedPlanId: approvalValue }));
    return 0;
}
async function runOperation(command, args, deps, config) {
    const parsed = parseWithOptions(args, { ...baseOptions(), enqueue: { type: "boolean" }, wait: { type: "boolean" }, job: { type: "string" } }, true);
    const b = base(parsed.values);
    if (b.help) {
        help(deps, b.json, COMMAND_HELP[command]);
        return 0;
    }
    if (deps.operate === undefined || parsed.positionals.length > (command === "raptor" ? 1 : 0) || command === "raptor" && parsed.positionals[0] !== "rebuild" || parsed.values.enqueue === parsed.values.wait)
        throw new CliInputError();
    ...
    output(deps, b.json, command, await deps.operate(config, { command, action, ...(jobId === undefined ? {} : { jobId }) }));
    return 0;
}
async function runInspect(args, deps, config) {
    const parsed = parseWithOptions(args, { ...baseOptions(), id: { type: "string" }, type: { type: "string" }, limit: { type: "string" } });
    const b = base(parsed.values);
    if (b.help) {
        help(deps, b.json, COMMAND_HELP.inspect);
        return 0;
    }
    ...
    output(deps, b.json, "inspect", await deps.inspect(config, { ...(ids === undefined ? {} : { ids }), ...(recordTypes === undefined ? {} : { recordTypes }), limit: positiveLimit(parsed.values.limit) }));
    return 0;
}
async function runForget(args, deps, config) {
    ...
    const b = base(parsed.values);
    if (b.help) {
        help(deps, b.json, COMMAND_HELP.forget);
        return 0;
    }
    if (deps.forgetPlan === undefined)
        throw new CliInputError();
    const selectors = ["current", "observation", "episode", "content", "state"];
    ...
    if (planId === undefined && selection === undefined)
        throw new CliInputError();
    ...
    if (planId !== undefined && planId !== plan.id)
        throw new CliApprovalError("forget plan does not match current state");
    if (parsed.values.approve === undefined) {
        output(deps, b.json, "forget", plan);
        return 0;
    }
    if (deps.forgetApply === undefined)
        throw new CliInputError();
    output(deps, b.json, "forget", await deps.forgetApply(config, { plan, approvedPlanId: approvalValue }));
    return 0;
}
async function runCommand(command, args, deps) {
    if (args.includes("--help") || args.includes("-h")) {
        help(deps, requestedJson(args), COMMAND_HELP[command]);
        return 0;
    }
    if (command === "status" || command === "init" || command === "project" || command === "privacy" || command === "curate" || command === "raptor" || command === "reconcile" || command === "inspect" || command === "forget") {
        ...
        if (command === "status") {
            const parsed = parseWithOptions(args, baseOptions());
            const b = base(parsed.values);
            output(deps, b.json, command, await deps.status(config));
            return 0;
        }
        if (command === "init")
            return runInit(args, deps, config);
        if (command === "project")
            return runProject(args, deps, config);
        if (command === "privacy")
            return runPrivacy(args, deps, config);
        if (command === "inspect")
            return runInspect(args, deps, config);
        if (command === "forget")
            return runForget(args, deps, config);
...
function entryPointUrl(value) { if (value === undefined || value === "")
    return ""; try {
