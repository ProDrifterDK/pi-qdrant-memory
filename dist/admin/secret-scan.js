...
function credentialIdentifier(value) {
    ...
    return CREDENTIAL_SUFFIXES.some((suffix) => key === suffix || key.endsWith(`_${suffix}`) || key.endsWith(`-${suffix}`));
}
...
function hasSecretAssignment(text, pattern, keyIndex) {
    pattern.lastIndex = 0;
    for (let match = pattern.exec(text); match !== null; match = pattern.exec(text)) {
        const key = match[keyIndex];
        if (key === undefined || !credentialIdentifier(key))
            continue;
        const rightHandSide = text.slice(pattern.lastIndex, endOfLine(text, pattern.lastIndex));
        if (!placeholder(rightHandSide))
            return true;
    }
    return false;
}
...
export function containsSecret(text) {
    ...
    if (STANDALONE_BEARER.test(text))
        return true;
    return (hasSecretAssignment(text, UNQUOTED_ASSIGNMENT, 1) ||
