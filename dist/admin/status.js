...
function safeCount(value, fallback = 0) { return value !== undefined && Number.isSafeInteger(value) && value >= 0 ? value : fallback; }
function safeDate(value) { if (value === null || value === undefined)
    return null; return typeof value === "string" && Number.isFinite(Date.parse(value)) ? new Date(value).toISOString() : null; }
function safeHash(value) { return value !== undefined && /^[a-f0-9]{64}$/u.test(value) ? value : null; }
function safeRedactedId(value) { return typeof value === "string" && value.length > 0 && value.length <= 256 && !/(?:api[-_]?key|token|secret|password)/iu.test(value) ? value : null; }
function endpointOrigin(value) { try {
...
export async function memoryStatus(config, deps = {}) {
    ...
    if (deps.audit !== undefined) {
        ...
        if (audit.policy !== undefined)
            base.policy = { hash: safeHash(audit.policy.hash), mismatch: audit.policy.mismatch === true };
        if (audit.outbox !== undefined)
            base.outbox = { jobs: safeCount(audit.outbox.jobs), bytes: safeCount(audit.outbox.bytes), oldestAt: safeDate(audit.outbox.oldestAt), failures: safeCount(audit.outbox.failures) };
        if (audit.coverage !== undefined)
            base.coverage = { missing: safeCount(audit.coverage.missing), oldestAt: safeDate(audit.coverage.oldestAt), lastReconcileAt: safeDate(audit.coverage.lastReconcileAt) };
        if (audit.jobs !== undefined)
            base.jobs = { queued: safeCount(audit.jobs.queued), leased: safeCount(audit.jobs.leased), failed: safeCount(audit.jobs.failed) };
        ...
        if (audit.privacy !== undefined) {
            const revoked = Object.freeze((audit.privacy.revokedDestinationIds ?? []).map(safeRedactedId).filter((value) => value !== null).sort());
            base.privacy = { ...base.privacy, ...(audit.privacy.epoch === undefined ? {} : { epoch: safeCount(audit.privacy.epoch) }), revokedDestinationIds: revoked };
        }
        if (audit.records !== undefined) {
            const entries = Object.entries(audit.records).filter(([key, value]) => /^[a-z_]+$/u.test(key) && Number.isSafeInteger(value) && value >= 0).map(([key, value]) => [key, value]);
            entries.sort((left, right) => left[0].localeCompare(right[0]));
            base.recordCounts = Object.freeze(Object.fromEntries(entries));
        }
        if (audit.embeddingHealthy !== undefined)
            base.embeddingHealth = audit.embeddingHealthy === true;
        if (audit.dedicatedLlmAvailable !== undefined)
            base.dedicatedLlmAvailable = audit.dedicatedLlmAvailable === true;
        if (audit.fallbackLlmAvailable !== undefined)
            base.fallbackLlmAvailable = audit.fallbackLlmAvailable === true;
        if (audit.lastErrorCategory !== undefined)
            base.lastErrorCategory = audit.lastErrorCategory === null ? null : safeRedactedId(audit.lastErrorCategory);
    }
    ...
    try {
        ...
        base.lastErrorCategory = "health_unavailable";
    }
    try {
        const info = await statusCollectionInfo(statusOptions, deps.fetchImpl);
        base.destination.exists = true;
        base.destination.pointCount = info.pointsCount;
        base.metadata = { ...base.metadata, pointCount: info.pointsCount };
        base.qdrant.destinationHealthy = false;
        ...
        if (!base.destination.healthy)
            base.lastErrorCategory = "metadata_mismatch";
        if (base.destination.healthy) {
            try {
                ...
                if (controlPoints.length !== 1)
                    throw new Error("control readback is ambiguous");
                ...
                const countTypes = ["episode", "curated_memory", "curated_current", "raptor_summary", "job", "lease"];
                const counts = {};
                for (const recordType of countTypes) {
                    try {
                        const purpose = ["job", "lease"].includes(recordType) ? "internal" : "query";
                ...
                base.lastErrorCategory = "control_mismatch";
            }
        }
    }
    catch (error) {
        ...
        else if (base.lastErrorCategory === null)
            base.lastErrorCategory = "destination_unavailable";
    }
    return base;
}
