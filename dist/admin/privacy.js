...
function validHost(value) {
    if (value !== "pi" && value !== "prime")
        throw new TypeError("Privacy owner host is invalid");
}
...
function instant(value) {
    ...
    if (!Number.isFinite(parsed.getTime()))
        throw new TypeError("Privacy request time is invalid");
    return parsed.toISOString();
}
function normalizedDestinations(value) {
    const entries = value === undefined ? [] : [...value];
    if (entries.length > MAX_DESTINATIONS)
        throw new TypeError("Privacy destinations are unbounded");
    const result = entries.map((entry, index) => safeText(entry, `destinationIds[${index}]`, 256));
    result.sort();
    for (let index = 1; index < result.length; index += 1)
        if (result[index] === result[index - 1])
            throw new TypeError("Privacy destinations must be unique");
    return result;
}
function controlValues(input) {
    ...
    validHost(ownerHost);
    if (ownerHost !== input.ownerHost)
        throw new TypeError("Privacy control owner mismatch");
...
export function planPrivacyRevoke(input) {
    ...
    const body = { destinationIds, fromPrivacyEpoch: epoch, invalidatesGeneration: true, inFlightCallsCannotBeRevoked: true, operation: "privacy_revoke", ownerHost, reason, requestedAt, toPrivacyEpoch: epoch + 1 };
...
function planMatches(plan) {
    ...
    if (id !== sha256Hex(canonicalStringify(body)) || plan.operation !== "privacy_revoke" || plan.drainsWorkers !== true || plan.invalidatesGeneration !== true || plan.inFlightCallsCannotBeRevoked !== true)
        throw new AdminPlanError("Privacy revoke plan is invalid");
    validHost(plan.ownerHost);
    validEpoch(plan.fromPrivacyEpoch, "plan.fromPrivacyEpoch");
    validEpoch(plan.toPrivacyEpoch, "plan.toPrivacyEpoch");
    if (plan.toPrivacyEpoch !== plan.fromPrivacyEpoch + 1)
        throw new AdminPlanError("Privacy revoke plan epoch is invalid");
...
export async function revokePrivacy(input, deps = {}) {
    planMatches(input.plan);
    if (input.approvedPlanId !== input.plan.id)
        throw new AdminPlanError("Privacy revoke approval does not match the plan");
    const now = input.now ?? Date.now();
    if (!Number.isFinite(now))
        throw new TypeError("Privacy revoke clock is invalid");
    const before = deps.readControl === undefined ? undefined : await deps.readControl();
    if (before !== undefined && (before.ownerHost !== input.plan.ownerHost || before.privacyEpoch !== input.plan.fromPrivacyEpoch))
        throw new AdminPlanError("Privacy revoke plan is stale");
    let retiredEpoch = input.plan.fromPrivacyEpoch;
    if (deps.beginDrain !== undefined) {
        const drained = await deps.beginDrain({ now });
        if (typeof drained === "object" && drained !== null && "coordinationPolicyEpoch" in drained && typeof drained.coordinationPolicyEpoch === "number")
            retiredEpoch = drained.coordinationPolicyEpoch;
    }
    if (deps.waitForQuiescence !== undefined)
        await deps.waitForQuiescence({ retiredEpoch, ...(input.signal === undefined ? {} : { signal: input.signal }) });
    ...
    if (deps.recordRevocations !== undefined)
        await deps.recordRevocations({ destinationIds: input.plan.destinationIds, privacyEpoch: input.plan.toPrivacyEpoch, reason: input.plan.reason, ...(input.signal === undefined ? {} : { signal: input.signal }) });
    if (deps.invalidateGeneration !== undefined)
        await deps.invalidateGeneration();
    if (deps.reconcile !== undefined)
        await deps.reconcile();
    const reread = deps.rereadControl ?? deps.readControl;
    if (reread === undefined)
        throw new Error("Privacy revoke requires a durable post-write reread");
    const after = await reread();
    if (after.ownerHost !== input.plan.ownerHost || after.privacyEpoch !== input.plan.toPrivacyEpoch || after.activeGeneration !== null || input.plan.destinationIds.some((id) => !after.revokedDestinationIds.includes(id)))
        throw new Error("Privacy revoke barrier was not confirmed");
