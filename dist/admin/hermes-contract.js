...
function validOrdinaryDenseArray(value, validElement) {
...
function validDenseVector(value) {
    return (validOrdinaryDenseArray(value, (component) => typeof component === "number" && Number.isFinite(component)) && value.length > 0);
}
function validTags(value) {
    return validOrdinaryDenseArray(value, (tag) => typeof tag === "string");
}
...
function invalidSafetyFlag(payload, field) {
    if (!hasOwn(payload, field))
        return false;
    return payload[field] !== false;
}
export function validateHermesPoint(point) {
    const candidate = point;
    if (!isRecord(candidate) || !validId(candidate.id))
        return { eligible: false, reason: "id" };
    if (!validDenseVector(candidate.vector))
        return { eligible: false, reason: "vector" };
    ...
    if (hasOwn(payload, "model")) {
        if (typeof payload.model !== "string")
            return { eligible: false, reason: "model" };
        if (payload.model.length > 0)
            model = payload.model;
    }
    if (hasOwn(payload, "project_path")) {
        if (typeof payload.project_path !== "string")
    ...
    if (invalidSafetyFlag(payload, "stale"))
        return { eligible: false, reason: "stale" };
    if (invalidSafetyFlag(payload, "requires_review"))
        return { eligible: false, reason: "review-required" };
...
//# sourceMappingURL=hermes-contract.js.map