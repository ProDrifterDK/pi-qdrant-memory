...
import { assertBootstrapControl, collectionControlPoint, collectionMetadataPoint, collectionVectors, isPhysicalPointId } from "../qdrant/schema.js";
import { expectedQdrantCollection, readPolicy, validatePurpose } from "../qdrant/client.js";
...
function freezeOptions(input) {
    ...
    const snapshot = { baseUrl: baseUrlValue, collection, ownerHost, timeoutMs };
    if (apiKey !== undefined)
...
class RestQdrantAdminClient extends RestQdrantReadClient {
    ...
    async createCollection() { const response = await fetchJson(collectionPath(restState(this).options), { method: "PUT", headers: headers(restState(this).options.apiKey, true), body: JSON.stringify({ vectors: collectionVectors(), ...(restState(this).options.replicationFactor === undefined ? {} : { replication_factor: restState(this).options.replicationFactor }), ...(restState(this).options.writeConsistencyFactor === undefined ? {} : { write_consistency_factor: restState(this).options.writeConsistencyFactor }) }) }, requestOptions(restState(this).options, restState(this).fetchImpl)); const result = envelope(response); if (result !== true && !(isRecord(result) && ["acknowledged", "completed", "ok"].includes(String(result.status))))
    ...
    async createPayloadIndex(field, schema) { if (!/^[a-z][a-z0-9_]{0,127}$/u.test(field) || !["keyword", "integer", "datetime", "text"].includes(schema))
        failInput("Payload index declaration is invalid"); const url = new URL(collectionPath(restState(this).options, "/index")); url.searchParams.set("wait", "true"); const response = await fetchJson(url.toString(), { method: "PUT", headers: headers(restState(this).options.apiKey, true), body: JSON.stringify({ field_name: field, field_schema: schema }) }, requestOptions(restState(this).options, restState(this).fetchImpl)); updateEnvelope(response); }
    async deletePoints(ids) { if (!Array.isArray(ids) || ids.length === 0 || ids.length > 1024 || ids.some((id) => !validId(id)))
        failInput("Delete IDs are invalid"); const url = new URL(collectionPath(restState(this).options, "/points/delete")); url.searchParams.set("wait", "true"); const response = await fetchJson(url.toString(), { method: "POST", headers: headers(restState(this).options.apiKey, true), body: JSON.stringify({ points: ids }) }, requestOptions(restState(this).options, restState(this).fetchImpl)); updateEnvelope(response); }
    async insertMetadataPoint(owner) { if (owner !== this.ownerHost)
        failInput("Metadata owner mismatch"); const response = await fetchJson(pointWriteUrl(restState(this).options), { method: "PUT", headers: headers(restState(this).options.apiKey, true), body: JSON.stringify({ points: [collectionMetadataPoint(owner)], update_mode: "insert_only" }) }, requestOptions(restState(this).options, restState(this).fetchImpl)); updateEnvelope(response); }
    async insertInitialControlPoint(control) { assertBootstrapControl(control, this.ownerHost); const response = await fetchJson(pointWriteUrl(restState(this).options), { method: "PUT", headers: headers(restState(this).options.apiKey, true), body: JSON.stringify({ points: [collectionControlPoint(control)], update_mode: "insert_only" }) }, requestOptions(restState(this).options, restState(this).fetchImpl)); updateEnvelope(response); }
    async serverInfo() { const response = await fetchJson(`${restState(this).options.baseUrl}/`, { method: "GET", headers: headers(restState(this).options.apiKey) }, requestOptions(restState(this).options, restState(this).fetchImpl)); if (!isRecord(response) || typeof response.version !== "string" || !validQdrantVersion(response.version))
...
function adminClientFor(options, fetchImpl) {
    return new RestQdrantAdminClient({ ...options, apiKey: options.apiKey, fetchImpl });
}
function readClientFor(options, fetchImpl) {
    return new RestQdrantReadClient({ ...options, fetchImpl });
}
/** @internal */ export async function adminServerInfo(options, fetchImpl) { return adminClientFor(options, fetchImpl).serverInfo(); }
/** @internal */ export async function adminCollectionInfo(options, fetchImpl) { return adminClientFor(options, fetchImpl).collectionInfo(); }
/** @internal */ export async function adminCreateCollection(options, fetchImpl) { return adminClientFor(options, fetchImpl).createCollection(); }
/** @internal */ export async function adminCreatePayloadIndex(options, fetchImpl, field, schema) { return adminClientFor(options, fetchImpl).createPayloadIndex(field, schema); }
/** @internal */ export async function adminInsertMetadataPoint(options, fetchImpl, owner) { return adminClientFor(options, fetchImpl).insertMetadataPoint(owner); }
/** @internal */ export async function adminInsertInitialControlPoint(options, fetchImpl, control) { return adminClientFor(options, fetchImpl).insertInitialControlPoint(control); }
/** @internal */ export async function adminRetrieve(options, fetchImpl, ids, policy, includeVector = false) { return adminClientFor(options, fetchImpl).retrieve(ids, policy, { includeVector }); }
