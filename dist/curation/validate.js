...
export const CURATION_CATEGORIES = ["preference", "correction", "convention", "fact", "failure", "learning"];
export const CURATION_SCOPES = ["project", "session", "host", "global"];
const MAX_ITEMS = 32;
...
function fail(message) { throw new TypeError(`Curation result is invalid: ${message}`); }
...
function bounded(name, value, max = MAX_FIELD) {
    if (typeof value !== "string" || value.length === 0 || value.length > max)
        fail(`${name} must be a bounded non-empty string`);
    if (SECRETISH.test(value))
        fail(`${name} must be redacted`);
    return value;
}
function itemId(name, value) {
    const id = bounded(name, value, 512);
    if (!ID.test(id))
        fail(`${name} must be an exact episode id`);
    return id;
}
function boundedList(name, value, max) {
    if (!Array.isArray(value) || value.length === 0 || value.length > max)
        fail(`${name} must be a bounded non-empty list`);
    return value;
}
/** Owned canonical clone constructed exclusively from own data descriptors. */
function ownedPlainClone(value) {
    ...
    const clone = (candidate) => {
        ...
        if (typeof candidate === "number") {
            if (!Number.isFinite(candidate))
                fail("input value contains a non-finite number");
            return candidate;
        }
        if (typeof candidate !== "object" || nodeTypes.isProxy(candidate) || active.has(candidate))
            fail("input is not a plain acyclic JSON graph");
        active.add(candidate);
        try {
            if (Object.getOwnPropertySymbols(candidate).length > 0)
                fail("input contains symbol keys");
            if (Array.isArray(candidate)) {
                if (Object.getPrototypeOf(candidate) !== Array.prototype)
                    fail("input array prototype is invalid");
                ...
                if (lengthDescriptor === undefined || !("value" in lengthDescriptor) || !Number.isSafeInteger(lengthDescriptor.value) || lengthDescriptor.value < 0 || lengthDescriptor.value > 4096 || Object.getOwnPropertyNames(candidate).length !== lengthDescriptor.value + 1)
                    fail("input array is sparse or unbounded");
                const result = [];
                for (let index = 0; index < lengthDescriptor.value; index += 1) {
                    const descriptor = Object.getOwnPropertyDescriptor(candidate, String(index));
                    if (descriptor === undefined || !("value" in descriptor) || descriptor.enumerable !== true)
                        fail("input array contains an accessor or hole");
    ...
    try {
        ...
        if (error instanceof TypeError && error.message.startsWith("Curation result is invalid:"))
            throw error;
        fail("input is not canonical JSON");
    }
}
function validateItem(value, ctx, maxEvidence) {
    if (!isPlainRecord(value))
        fail("items must be plain objects");
    const allowed = new Set(["category", "scope", "subject", "predicate", "value", "text", "evidence", "confidence"]);
    for (const key of Object.keys(value))
        if (!allowed.has(key))
            fail(`unknown item field ${key}`);
    if (Object.getOwnPropertySymbols(value).length > 0)
        fail("item fields must not be symbols");
    ...
    if (typeof category !== "string" || !CURATION_CATEGORIES.includes(category))
        fail("unknown category");
    if (typeof scope !== "string" || !CURATION_SCOPES.includes(scope))
        fail("unknown scope");
    ...
    if (itemValue !== undefined) {
        let serialized;
        try {
            serialized = canonicalStringify(itemValue);
        }
        catch {
    ...
    if (text !== undefined)
        bounded("text", text, MAX_TEXT_CHARS);
    if (itemValue === undefined && text === undefined)
        fail("item requires a value or text");
    ...
    if (new Set(evidenceIds).size !== evidenceIds.length)
        fail("evidence must not repeat");
    for (const id of evidenceIds)
        if (!ctx.knownEpisodeIds.has(id))
            fail(`evidence episode ${id} is not in the explicit membership`);
    // Tool output can never supply direct evidence for a standing instruction.
    if (category === "preference" || category === "correction") {
        if (!evidenceIds.some((id) => ctx.directUserEpisodeIds.has(id)))
            fail("direct user evidence is required for preferences and corrections");
    }
    ...
    if (itemValue !== undefined)
        item.value = Object.freeze(ownedPlainClone(itemValue));
    if (text !== undefined)
        item.text = text;
    if (confidence !== undefined)
 ...
 */
export function validateCurationResult(input, ctx) {
    const directUserEpisodeIds = ctx.directUserEpisodeIds;
    const knownEpisodeIds = ctx.knownEpisodeIds;
    if (directUserEpisodeIds === null || typeof directUserEpisodeIds !== "object" || knownEpisodeIds === null || typeof knownEpisodeIds !== "object")
        fail("validation context is invalid");
    const maxItems = ctx.maxItems ?? MAX_ITEMS;
    const maxEvidence = ctx.maxEvidence ?? MAX_EVIDENCE;
    if (!Number.isSafeInteger(maxItems) || maxItems < 1 || maxItems > MAX_ITEMS || !Number.isSafeInteger(maxEvidence) || maxEvidence < 1 || maxEvidence > MAX_EVIDENCE)
        fail("validation bounds are invalid");
    ...
    if (Object.getOwnPropertySymbols(input).length > 0)
        fail("top-level fields must not be symbols");
    const ownedInput = ownedPlainClone(input);
    if (!isPlainRecord(ownedInput))
        fail("input must be canonical JSON");
    const rawItems = ownedInput.items;
    if (!Array.isArray(rawItems) || rawItems.length > maxItems)
        fail("items must be a bounded list");
    ...
    let items;
    try {
        items = Array.from(rawItems);
        canonicalStringify(items);
    }
    catch {
        fail("items must be canonical JSON");
    }
 ...
 */
export function assertPersistableCurationResult(result, scan) {
    if (!isPlainRecord(result) || !Array.isArray(result.items))
        fail("persistable result is invalid");
    for (const item of result.items) {
        let serialized;
        try {
            serialized = canonicalStringify(item);
        }
        catch {
            fail("persistable item is not canonical JSON");
        }
        const checked = redactAndScan({ text: serialized, maxChars: 65_536, homeDir: "/", ...(scan === undefined ? {} : { scan }) });
        if (checked.dropped || checked.secretScan !== "passed" || checked.redactionStatus !== "unchanged" || checked.text !== serialized)
            fail("result contains unsafe persistable data");
    }
    return result;
}
/**
 ...
 */
export function parseStrictCurationJson(text) {
    if (typeof text !== "string" || text.length === 0 || text.length > 65_536)
        throw new TypeError("Curation output must be a bounded non-empty string");
    if (!/^\s*[{[]/u.test(text) || !/[}\]]\s*$/u.test(text))
        throw new TypeError("Curation output must be a bare JSON object/array without fences or prose");
    assertNoDuplicateKeys(text);
    let parsed;
    try {
        parsed = JSON.parse(text, (key, value) => {
            if (key === "__proto__" || key === "constructor" || key === "prototype")
                throw new TypeError("prototype key");
            return value;
        });
    }
    catch {
        throw new TypeError("Curation output is not strict JSON");
    }
...
function assertNoDuplicateKeys(text) {
    ...
    while (index < length) {
        ...
        if (ch === "{") {
            ...
            index += 1;
            let cursor = index;
            while (cursor < length && /\s/u.test(text[cursor]))
                cursor += 1;
