import { contentId, observationId, stateKey, curatedCurrentId, validateEffectiveOrder, MAX_SESSION_SEQUENCE } from "../domain/ids.js";
import { canonicalStringify } from "../domain/canonical.js";
import { gateCuratedEgressText } from "../security/egress.js";
export function primaryEvidence(evidence) { if (evidence.length === 0)
    throw new TypeError("Projection evidence is empty"); let primary = evidence[0]; for (const ep of evidence) {
    const at = Date.parse(ep.eventAt), prior = Date.parse(primary.eventAt);
    if (at > prior || (at === prior && ep.id > primary.id))
        primary = ep;
} return primary; }
export function projectEffectiveOrder(evidence, value) { const sessions = new Set(evidence.map((ep) => ep.sessionId)); if (sessions.size === 1 && evidence.every((ep) => ep.sessionSequence !== undefined)) {
    const primary = primaryEvidence(evidence);
    const sequence = Math.max(...evidence.map((ep) => ep.sessionSequence));
    if (!Number.isSafeInteger(sequence) || sequence < 0 || sequence > MAX_SESSION_SEQUENCE)
        throw new TypeError("Projection session sequence is invalid");
    return Object.freeze({ kind: "session", sessionId: primary.sessionId, sequence, eventAt: primary.eventAt, episodeId: primary.id, contentId: value });
} const primary = primaryEvidence(evidence); return [primary.eventAt, primary.id, value]; }
export function projectCurationText(item) {
...
export function projectCurationItem(host, policyHash, policyEpoch, item, episodes) {
    const evidence = item.evidence.map((id) => episodes.get(id));
    if (evidence.some((ep) => ep === undefined))
        throw new TypeError("Projection evidence is missing");
    const present = evidence;
    let projectId;
    if (item.scope === "project") {
        const set = new Set(present.map((ep) => ep.projectId));
        if (set.size !== 1)
            throw new TypeError("Project-scoped evidence spans multiple projects");
        projectId = [...set][0];
    }
    if (item.scope === "session") {
        const set = new Set(present.map((ep) => ep.sessionId));
        if (set.size !== 1)
            throw new TypeError("Session-scoped evidence spans multiple sessions");
    }
    const state = stateKey({ host, scope: item.scope, ...(projectId === undefined ? {} : { projectId }), category: item.category, subject: item.subject, predicate: item.predicate });
    const value = item.value !== undefined ? item.value : item.text;
    const content = contentId(policyHash, state, value);
    const order = projectEffectiveOrder(present, content);
...
function laterDeterministic(left, right) {
    const leftSession = !Array.isArray(left.effectiveOrder) && typeof left.effectiveOrder === "object" ? left.effectiveOrder : null;
    const rightSession = !Array.isArray(right.effectiveOrder) && typeof right.effectiveOrder === "object" ? right.effectiveOrder : null;
...
export function projectConflictAggregate(membersInput) {
    if (!Array.isArray(membersInput) || membersInput.length < 2)
        throw new TypeError("Conflict aggregate requires at least two members");
    const members = [...membersInput].sort((a, b) => a.id.localeCompare(b.id));
    if (new Set(members.map((member) => member.id)).size !== members.length)
        throw new TypeError("Conflict aggregate members repeat");
    const first = members[0];
    if (members.some((member) => member.stateKey !== first.stateKey || member.ownerHost !== first.ownerHost || member.processingPolicyId !== first.processingPolicyId || member.expiresAt !== first.expiresAt || member.coordinationPolicyHash !== first.coordinationPolicyHash || member.coordinationPolicyEpoch !== first.coordinationPolicyEpoch || member.privacyEpoch !== first.privacyEpoch))
        throw new TypeError("Conflict aggregate envelope mismatch");
    const representativeMap = new Map();
    for (const member of members) {
        const previous = representativeMap.get(member.contentId);
        if (previous === undefined || laterDeterministic(previous, member) === member)
            representativeMap.set(member.contentId, member);
    }
    ...
    if (representatives.length < 2)
        throw new TypeError("Conflict aggregate requires distinct logical content");
    let latest = representatives[0];
    for (const member of representatives.slice(1))
        latest = laterDeterministic(latest, member);
...
export function effectiveOrderTuple(value) {
...
export function compareProjectionOrders(a, b, skew) {
    if (!Number.isSafeInteger(skew) || skew < 0)
        throw new TypeError("Clock skew is invalid");
    validateEffectiveOrder(a);
    validateEffectiveOrder(b);
    if (canonicalStringify(a) === canonicalStringify(b))
        return "equal";
    const as = !Array.isArray(a) && typeof a === "object" ? a : null;
    const bs = !Array.isArray(b) && typeof b === "object" ? b : null;
    if (as !== null && bs !== null && as.sessionId === bs.sessionId) {
        if (as.sequence !== bs.sequence)
            return as.sequence < bs.sequence ? "before" : "after";
