...
import { createOutbox, parseOutboxJob, resolveOutboxNodeId } from "../../src/outbox/store.js";
...
function episode(current: ProcessingPolicy, id = "00000000-0000-5000-8000-000000000001", text = "already [token redacted]"): EpisodeRecord {
  const pending: EpisodeRecord = {
    ...
    destinationId: "qdrant:local", status: "active", redactionStatus: "unchanged", secretScan: "passed", text,
  };
...
describe("Task 5 durable outbox", () => {
  it("resolves the stable derived node identity without creating a producer", async () => {
    const homeDir = await mkdtemp(join(tmpdir(), "task5-node-resolution-"));
    const env = { PRIME_AGENT_CODING_AGENT_DIR: join(homeDir, ".prime", "agent") };
    const input = { host: "prime" as const, homeDir, env, machineId: "machine-test", sharedFilesystem: false };
    const nodeId = await resolveOutboxNodeId(input);
    const nodePath = join(env.PRIME_AGENT_CODING_AGENT_DIR, "pi-qdrant-memory", "outbox", nodeId);
    expect(await readdir(nodePath)).toEqual(["node.json"]);
    const outbox = await createOutbox(input);
    expect(outbox.nodeId).toBe(nodeId);
    expect((await readdir(nodePath)).sort()).toEqual(["node.json", outbox.producerUuid].sort());
  });
  ...
  it("adopts only canonical closed or stale producers and performs expiry checks before returning", async () => {
    ...
    try {
      ...
      const recoveryBytes = await readFile(join(outbox.producerPath, "recovery.json"), "utf8"); const recovery = JSON.parse(recoveryBytes);
      expect(recovery).toMatchObject({ version: 1, kind: "recovery_rotation", producerUuid: outbox.producerUuid, recoveredAt: clock }); expect(recovery.auditHash).toMatch(/^[a-f0-9]{64}$/u);
      await expect(stat(join(outbox.producerPath, "quarantine", `${job.id}.json`))).rejects.toMatchObject({ code: "ENOENT" });
      expect(await stat(join(outbox.producerPath, "quarantine", `${job.id}.expired.json`))).toBeDefined();
      expect(processor.process).not.toHaveBeenCalled();
      await delivery.adopt(outbox.producerPath); const secondRecovery = JSON.parse(await readFile(join(outbox.producerPath, "recovery.json"), "utf8")); expect(secondRecovery.recoveredAt).toBe(clock + 1); expect(secondRecovery.auditHash).not.toBe(recovery.auditHash);
    } finally { await rm(homeDir, { recursive: true, force: true }); }
  });
...
${await readFile("src/outbox/delivery.ts", "utf8")}`;
  ...
  it("converges concurrent delivered and quarantine terminals in both interleavings and APIs", async () => {
    for (const api of ["delivery", "store"] as const) for (const ordering of ["quarantine-first", "delivered-first"] as const) {
      const homeDir = await mkdtemp(join(tmpdir(), `task5-terminal-race-${api}-${ordering}-`)); let source = ""; let destination = ""; let resolveQuarantineReached!: () => void; let resolveDeliveredAtRemove!: () => void; let resolveDeliveredRemoved!: () => void; let resolveDeliveredProcessorReached!: () => void; const quarantineReached = new Promise<void>((resolveValue) => { resolveQuarantineReached = resolveValue; }); const deliveredAtRemove = new Promise<void>((resolveValue) => { resolveDeliveredAtRemove = resolveValue; }); const deliveredRemoved = new Promise<void>((resolveValue) => { resolveDeliveredRemoved = resolveValue; }); const deliveredProcessorReached = new Promise<void>((resolveValue) => { resolveDeliveredProcessorReached = resolveValue; }); let processorCount = 0; let resolveBothProcessors!: () => void; const bothProcessors = new Promise<void>((resolveValue) => { resolveBothProcessors = resolveValue; });
      const quarantineFs = { ...nodeFs, link: async (existingPath: Parameters<typeof nodeFs.link>[0], newPath: Parameters<typeof nodeFs.link>[1]) => { if (String(existingPath) === source && String(newPath) === destination) { if (api === "store") await deliveredProcessorReached; if (ordering === "delivered-first") { resolveQuarantineReached(); await deliveredRemoved; return nodeFs.link(existingPath, newPath); } const result = await nodeFs.link(existingPath, newPath); resolveQuarantineReached(); return result; } return nodeFs.link(existingPath, newPath); }, rm: async (path: Parameters<typeof nodeFs.rm>[0], options?: Parameters<typeof nodeFs.rm>[1]) => { if (String(path) === source && ordering === "quarantine-first") await deliveredAtRemove; return options === undefined ? nodeFs.rm(path) : nodeFs.rm(path, options); } };
      ...
      try {
        ...
        const deliveredProcessor: OutboxJobProcessor = { process: vi.fn(async () => { if (api === "delivery") { processorCount += 1; if (processorCount === 2) resolveBothProcessors(); await bothProcessors; } else { resolveDeliveredProcessorReached(); if (ordering === "quarantine-first") await quarantineReached; } return { status: "delivered" }; }) }; const delivered = createOutboxDelivery({ outboxRoot: outbox.root, producerPath: outbox.producerPath, processor: deliveredProcessor, now: () => Date.parse("2029-01-03T00:00:00.000Z"), maxClockSkewMs: 0, fs: deliveredFs });
        let deliveredPromise: Promise<unknown>; let quarantinePromise: Promise<unknown>; if (api === "delivery") { const quarantineProcessor: OutboxJobProcessor = { process: vi.fn(async () => { processorCount += 1; if (processorCount === 2) resolveBothProcessors(); await bothProcessors; return { status: "quarantined", category: "race" }; }) }; const quarantineDelivery = createOutboxDelivery({ outboxRoot: outbox.root, producerPath: outbox.producerPath, processor: quarantineProcessor, now: () => Date.parse("2029-01-03T00:00:00.000Z"), maxClockSkewMs: 0, fs: quarantineFs }); quarantinePromise = quarantineDelivery.deliver({}); deliveredPromise = delivered.deliver({}); } else { deliveredPromise = delivered.deliver({}); quarantinePromise = outbox.quarantine(job, "race"); } const settled = await Promise.allSettled([deliveredPromise, quarantinePromise]); expect(settled).toHaveLength(2);
      ...
      } finally { resolveQuarantineReached?.(); resolveDeliveredAtRemove?.(); resolveDeliveredRemoved?.(); resolveDeliveredProcessorReached?.(); resolveBothProcessors?.(); await rm(homeDir, { recursive: true, force: true }); }
    }
  }, 20_000);
...
`, { mode: 0o600 });
      ...
      await new Promise<void>((resolveReady, rejectReady) => { let settled = false; let timer: ReturnType<typeof setTimeout>; const finish = (error?: Error): void => { if (settled) return; settled = true; clearTimeout(timer); child!.stdout!.off("data", onData); child!.off("error", onError); child!.off("exit", onExit); if (error === undefined) resolveReady(); else rejectReady(error); }; const onData = (chunk: unknown): void => { stdout += String(chunk); if (stdout.includes("PAUSED")) finish(); }; const onError = (error: Error): void => finish(error); const onExit = (code: number | null): void => { if (!stdout.includes("PAUSED")) finish(new Error(`child exited before pause: ${code}: ${stderr}`)); }; timer = setTimeout(() => finish(new Error("child did not reach pre-temp pause")), 15_000); child!.stdout!.on("data", onData); child!.once("error", onError); child!.once("exit", onExit); });
